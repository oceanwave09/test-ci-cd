...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="radaid_cxcascr_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "radaid_cxcascr"))
        ...
        # add additional columns and prepare dataframe which match with radaid cervical scr delta lake table structure
        cxcascr_df = (
            ...
            .withColumn("scr_date", get_column(canonical_df, "scr_date"))
            .withColumn("scr_name", get_column(canonical_df, "scr_name"))
            .withColumn("scr_code", get_column(canonical_df, "scr_code"))
            .withColumn("scr_result", get_column(canonical_df, "scr_result"))
        ...
        cxcascr_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "radaid_cxcascr", opsgenie_api_key, file_details)
