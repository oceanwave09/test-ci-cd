...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="radaid_brcadiag_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "radaid_brcadiag"))
        ...
        # add additional columns and prepare dataframe which match with radaid breast diag delta lake table structure
        brcadiag_df = (
        ...
        brcadiag_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "radaid_brcadiag", opsgenie_api_key, file_details)
