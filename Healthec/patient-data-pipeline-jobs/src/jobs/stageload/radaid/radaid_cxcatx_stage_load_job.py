...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="radaid_cxcatx_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "radaid_cxcatx"))
        ...
        # add additional columns and prepare dataframe which match with radaid cervical tx delta lake table structure
        cxcatx_df = (
            ...
            .withColumn("tx_cons_date", get_column(canonical_df, "tx_cons_date"))
            .withColumn("tx_date", get_column(canonical_df, "tx_date"))
            .withColumn("tx_name", get_column(canonical_df, "tx_name"))
            .withColumn("tx_code", get_column(canonical_df, "tx_code"))
            .withColumn("stage", get_column(canonical_df, "stage"))
        ...
        cxcatx_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "radaid_cxcatx", opsgenie_api_key, file_details)
