...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="radaid_ptoutreach_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "radaid_ptoutreach"))
        ...
        # add additional columns and prepare dataframe which match with radaid pt outreach delta lake table structure
        ptoutreach_df = (
            ...
            .withColumn("outreach_type", get_column(canonical_df, "outreach_type"))
            .withColumn("pn_contact", get_column(canonical_df, "pn_contact"))
            .withColumn("pn_contact_date", get_column(canonical_df, "pn_contact_date"))
            .withColumn("pn_barrier", get_column(canonical_df, "pn_barrier"))
            .withColumn("outcome", get_column(canonical_df, "outcome"))
        ...
        ptoutreach_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "radaid_ptoutreach", opsgenie_api_key, file_details)
