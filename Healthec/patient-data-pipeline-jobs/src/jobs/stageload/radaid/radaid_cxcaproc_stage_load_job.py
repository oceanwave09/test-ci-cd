...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="radaid_cxcaproc_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "radaid_cxcaproc"))
        ...
        # add additional columns and prepare dataframe which match with radaid cervical proc delta lake table structure
        cxcaproc_df = (
            ...
            .withColumn("proc_date", get_column(canonical_df, "proc_date"))
            .withColumn("proc_name", get_column(canonical_df, "proc_name"))
            .withColumn("proc_code", get_column(canonical_df, "proc_code"))
            .withColumn("proc_result", get_column(canonical_df, "proc_result"))
        ...
        cxcaproc_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "radaid_cxcaproc", opsgenie_api_key, file_details)
