...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "cclf3"),
        )
        ...
        # add additional columns and prepare dataframe which match with cclf3 delta lake table structure
        cclf3_df = (
            ...
            .withColumn("clm_val_sqnc_num", to_int_udf(get_column(data_df, "clm_val_sqnc_num")))
            .withColumn("clm_prcdr_cd", get_column(data_df, "clm_prcdr_cd"))
            .withColumn(
                "clm_prcdr_prfrm_dt", f.to_timestamp(parse_date_time_udf(get_column(data_df, "clm_prcdr_prfrm_dt")))
            )
        ...
        # write data into delta lake `cclf3` table
        cclf3_df.write.format("delta").mode("append").save(delta_table_location)
