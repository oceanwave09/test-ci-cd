...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "cclf9"),
        )
        ...
        # add additional columns and prepare dataframe which match with cclf9 delta lake table structure
        cclf9_df = (
            data_df.withColumn("row_id", get_column(data_df, "row_id"))
            .withColumn("hicn_mbi_xref_ind", get_column(data_df, "hicn_mbi_xref_ind…"))
            .withColumn("crnt_num", get_column(data_df, "crnt_num"))
            .withColumn("prvs_num", get_column(data_df, "prvs_num"))
            .withColumn("prvs_id_efctv_dt", f.to_date(parse_date_udf(get_column(data_df, "prvs_id_efctv_dt"))))
            .withColumn("prvs_id_obslt_dt", f.to_date(parse_date_udf(get_column(data_df, "prvs_id_obslt_dt"))))
            .withColumn("bene_rrb_num", get_column(data_df, "bene_rrb_num"))
        ...
        # write data into delta lake `cclf9` table
        cclf9_df.write.format("delta").mode("append").save(delta_table_location)
