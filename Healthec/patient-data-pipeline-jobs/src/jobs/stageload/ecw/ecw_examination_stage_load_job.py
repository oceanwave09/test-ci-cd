...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_examination_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_examination"))
        ...
        # add additional columns and prepare dataframe which match with ecw examination delta lake table structure
        examination_df = (
        ...
        # write data into delta lake `ecw_examination` table
        examination_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_examination", opsgenie_api_key, file_details)
