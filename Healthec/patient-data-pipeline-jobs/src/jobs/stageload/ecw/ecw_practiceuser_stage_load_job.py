...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_practiceuser_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_practiceuser"))
        ...
        # add additional columns and prepare dataframe which match with ecw practiceuser delta lake table structure
        practiceuser_df = (
            ...
            .withColumn("user_id", get_column(canonical_df, "user_id"))
            .withColumn("provider_npi", get_column(canonical_df, "provider_npi"))
            .withColumn("user_type", get_column(canonical_df, "user_type"))
            .withColumn("user_first_name", get_column(canonical_df, "user_first_name"))
            .withColumn("user_last_name", get_column(canonical_df, "user_last_name"))
            ...
            .withColumn("user_status", get_column(canonical_df, "user_status"))
        ...
        # write data into delta lake `ecw_practiceuser` table
        practiceuser_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_practiceuser", opsgenie_api_key, file_details)
