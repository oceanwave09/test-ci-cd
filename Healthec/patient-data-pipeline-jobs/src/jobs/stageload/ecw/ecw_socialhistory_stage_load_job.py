...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_socialhistory_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_socialhistory"))
        ...
        # add additional columns and prepare dataframe which match with ecw socialhistory delta lake table structure
        socialhistory_df = (
            ...
            .withColumn("encounter_date", get_column(canonical_df, "encounter_date"))
            .withColumn("provider_npi", get_column(canonical_df, "provider_npi"))
            .withColumn("ecw_struct_id", get_column(canonical_df, "ecw_struct_id"))
            .withColumn("question", get_column(canonical_df, "question"))
            .withColumn("question_id", get_column(canonical_df, "question_id"))
            .withColumn("answers", get_column(canonical_df, "answers"))
            .withColumn("answers_id", get_column(canonical_df, "answers_id"))
            .withColumn("item_name", get_column(canonical_df, "item_name"))
            .withColumn("item_id", get_column(canonical_df, "item_id"))
            .withColumn("category_name", get_column(canonical_df, "category_name"))
            .withColumn("category_id", get_column(canonical_df, "category_id"))
            .withColumn("main_category_name", get_column(canonical_df, "main_category_name"))
            .withColumn("main_category_id", get_column(canonical_df, "main_category_id"))
        ...
        # write data into delta lake `ecw_socialhistory` table
        socialhistory_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_socialhistory", opsgenie_api_key, file_details)
