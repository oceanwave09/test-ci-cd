...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_familyhistory_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_familyhistory"))
        ...
        # add additional columns and prepare familyhistory delta lake table structure
        familyhistory_df = (
            ...
            .withColumn("relationship", get_column(canonical_df, "relationship"))
            .withColumn("status", get_column(canonical_df, "status"))
            .withColumn("age", get_column(canonical_df, "age"))
            .withColumn("date_of_birth", get_column(canonical_df, "date_of_birth"))
            .withColumn("icd_item_id", get_column(canonical_df, "icd_item_id"))
            .withColumn("icd_code", get_column(canonical_df, "icd_code"))
            .withColumn("icd_desc", get_column(canonical_df, "icd_desc"))
            .withColumn("diagnosed_year", get_column(canonical_df, "diagnosed_year"))
            .withColumn("diagnosed_age", get_column(canonical_df, "diagnosed_age"))
        ...
        # write data into delta lake `ecw_familyhistory` table
        familyhistory_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_familyhistory", opsgenie_api_key, file_details)
