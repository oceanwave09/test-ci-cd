...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_facility_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_facility"))
        ...
        # add additional columns and prepare dataframe which match with ecw facility delta lake table structure
        facility_df = (
            ...
            .withColumn("address_city", get_column(canonical_df, "address_city"))
            .withColumn("address_state", get_column(canonical_df, "address_state"))
            .withColumn("address_zip", get_column(canonical_df, "address_zip"))
        ...
        # write data into delta lake `ecw_facility` table
        facility_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_facility", opsgenie_api_key, file_details)
