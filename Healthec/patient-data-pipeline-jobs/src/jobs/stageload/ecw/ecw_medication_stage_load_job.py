...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_medication_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_medication"))
        ...
        # add additional columns and prepare dataframe which match with ecw medication delta lake table structure
        medication_df = (
            ...
            .withColumn("medication_origin_id", get_column(canonical_df, "medication_origin_id"))
            .withColumn("medication_code_system", get_column(canonical_df, "medication_code_system"))
            .withColumn("medication_code", get_column(canonical_df, "medication_code"))
            .withColumn("medication_description", get_column(canonical_df, "medication_description"))
            .withColumn("prescription_date", get_column(canonical_df, "prescription_date"))
            .withColumn("prescription_begin_date", get_column(canonical_df, "prescription_begin_date"))
            .withColumn("prescription_end_date", get_column(canonical_df, "prescription_end_date"))
            ...
            .withColumn("strength", get_column(canonical_df, "strength"))
            .withColumn("formulation", get_column(canonical_df, "formulation"))
            .withColumn("route", get_column(canonical_df, "route"))
            .withColumn("frequency", get_column(canonical_df, "frequency"))
            .withColumn("take", get_column(canonical_df, "take"))
            .withColumn("medication_status", get_column(canonical_df, "med_status"))
        ...
        # write data into delta lake `ecw_medication` table
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_medication", opsgenie_api_key, file_details)
