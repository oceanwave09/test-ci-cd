...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_problemlist_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_problemlist"))
        ...
        # add additional columns and prepare dataframe which match with ecw problemlist delta lake table structure
        problemlist_df = (
            ...
            .withColumn("asmt_id", get_column(canonical_df, "asmt_id"))
            .withColumn("onset_date", get_column(canonical_df, "onset_date"))
            ...
            .withColumn("pt_condition", get_column(canonical_df, "pt_condition"))
            .withColumn("risk", get_column(canonical_df, "risk"))
            .withColumn("added_date", get_column(canonical_df, "added_date"))
            .withColumn("resolved_on", get_column(canonical_df, "resolved_on"))
        ...
        # write data into delta lake `ecw_problemlist` table
        problemlist_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_problemlist", opsgenie_api_key, file_details)
