...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_orderandresult_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_orderandresult"))
        ...
        # add additional columns and prepare dataframe which match with ecw orderandresult delta lake table structure
        orderandresult_df = (
            ...
            .withColumn("order_type", get_column(canonical_df, "order_type"))
            .withColumn("item_id", get_column(canonical_df, "item_id"))
            .withColumn("lonic", get_column(canonical_df, "lonic"))
            .withColumn("cpt", get_column(canonical_df, "cpt"))
            .withColumn("order_id", get_column(canonical_df, "order_id"))
            ...
            .withColumn("provider_id", get_column(canonical_df, "provider_id"))
            .withColumn("encounter_id", get_column(canonical_df, "encounter_id"))
            .withColumn("provider_npi", get_column(canonical_df, "provider_npi"))
            .withColumn("order_date", get_column(canonical_df, "order_date"))
            .withColumn("order_description", get_column(canonical_df, "order_description"))
            .withColumn("result_description", get_column(canonical_df, "result_description"))
            .withColumn("collection_date", get_column(canonical_df, "collection_date"))
            .withColumn("result_date", get_column(canonical_df, "result_date"))
            .withColumn("result_text", get_column(canonical_df, "result_text"))
            .withColumn("lab_reviewed_flag", get_column(canonical_df, "lab_reviewed_flag"))
            .withColumn("reviewed_by_id", get_column(canonical_df, "reviewed_by_id"))
            .withColumn("result_numeric", get_column(canonical_df, "result_numeric"))
            .withColumn("ref_range", get_column(canonical_df, "ref_range"))
            .withColumn("hl_units", get_column(canonical_df, "hl_units"))
            .withColumn("result_modify_timestamp", get_column(canonical_df, "result_modify_timestamp"))
            .withColumn("lab_company_id", get_column(canonical_df, "lab_company_id"))
            .withColumn("lab_company_name", get_column(canonical_df, "lab_company_name"))
        ...
        # write data into delta lake `ecw_orderandresult` table
        orderandresult_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_orderandresult", opsgenie_api_key, file_details)
