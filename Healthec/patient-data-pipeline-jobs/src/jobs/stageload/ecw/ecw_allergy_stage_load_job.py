...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_allergy_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_allergy"))
        ...
        # add additional columns and prepare dataframe which match with ecw allergy delta lake table structure
        allergy_df = (
            ...
            .withColumn("allergies_verified", get_column(canonical_df, "allergies_verified"))
            .withColumn("allergen_type", get_column(canonical_df, "allergen_type"))
            .withColumn("allergy_description", get_column(canonical_df, "allergy_description"))
            .withColumn("allergy_reaction_text", get_column(canonical_df, "allergy_reaction_text"))
            .withColumn("allergy_status", get_column(canonical_df, "allergy_status"))
            .withColumn("allergy_id", get_column(canonical_df, "allergy_id"))
            .withColumn("modified_date", get_column(canonical_df, "modified_date"))
        ...
        allergy_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_allergy", opsgenie_api_key, file_details)
