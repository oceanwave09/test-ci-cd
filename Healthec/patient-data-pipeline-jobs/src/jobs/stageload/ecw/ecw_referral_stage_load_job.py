...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_referral_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_referral"))
        ...
        # add additional columns and prepare dataframe which match with ecw referral delta lake table structure
        referral_df = (
            ...
            .withColumn("referral_id", get_column(canonical_df, "referral_id"))
            ...
            .withColumn("speciality_code", get_column(canonical_df, "speciality_code"))
            .withColumn("speciality", get_column(canonical_df, "speciality"))
            .withColumn("referral_type", get_column(canonical_df, "referral_type"))
            .withColumn("referral_status", get_column(canonical_df, "referral_status"))
            .withColumn("referral_from_name", get_column(canonical_df, "referral_from_name"))
            .withColumn("referral_from_npi", get_column(canonical_df, "referral_from_npi"))
            .withColumn("referral_to_name", get_column(canonical_df, "referral_to_name"))
            .withColumn("referral_to_npi", get_column(canonical_df, "referral_to_npi"))
            .withColumn("facility_from", get_column(canonical_df, "facility_from"))
            .withColumn("facility_to", get_column(canonical_df, "facility_to"))
            .withColumn("referral_date", get_column(canonical_df, "referral_date"))
        ...
        # write data into delta lake `ecw_referral` table
        referral_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_referral", opsgenie_api_key, file_details)
