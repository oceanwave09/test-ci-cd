...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_procedurecode_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_procedurecode"))
        ...
        # add additional columns and prepare dataframe which match with ecw procedurecode delta lake table structure
        procedurecode_df = (
            ...
            .withColumn("facility_id", get_column(canonical_df, "facility_id"))
            .withColumn("service_date", get_column(canonical_df, "service_date"))
            .withColumn("cpt_code", get_column(canonical_df, "cpt_code"))
            .withColumn("cpt_code_type", get_column(canonical_df, "cpt_code_type"))
            .withColumn("modifier_1", get_column(canonical_df, "modifier_1"))
            .withColumn("modifier_2", get_column(canonical_df, "modifier_2"))
            .withColumn("modifier_3", get_column(canonical_df, "modifier_3"))
            .withColumn("modifier_4", get_column(canonical_df, "modifier_4"))
            .withColumn("icd_code_1", get_column(canonical_df, "icd_code_1"))
            .withColumn("icd_code_2", get_column(canonical_df, "icd_code_2"))
            .withColumn("icd_code_3", get_column(canonical_df, "icd_code_3"))
            .withColumn("icd_code_4", get_column(canonical_df, "icd_code_4"))
            .withColumn("place_of_service", get_column(canonical_df, "place_of_service"))
            .withColumn("pos_code_description", get_column(canonical_df, "pos_code_description"))
        ...
        # write data into delta lake `ecw_procedurecode` table
        procedurecode_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_procedurecode", opsgenie_api_key, file_details)
