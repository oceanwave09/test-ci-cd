...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_obgynhistory_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_obgynhistory"))
        ...
        # add additional columns and prepare dataframe which match with ecw obgynhistory delta lake table structure
        obgynhistory_df = (
        ...
        # write data into delta lake `ecw_obgynhistory` table
        obgynhistory_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_obgynhistory", opsgenie_api_key, file_details)
