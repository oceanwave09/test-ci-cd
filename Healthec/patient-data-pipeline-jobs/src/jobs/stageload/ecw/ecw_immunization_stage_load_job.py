...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_immunization_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_immunization"))
        ...
        # add additional columns and prepare dataframe which match with ecw immunization delta lake table structure
        immunization_df = (
            ...
            .withColumn("given_date", get_column(canonical_df, "given_date"))
            .withColumn("immunization_description", get_column(canonical_df, "immunization_description"))
            .withColumn("history_flag", get_column(canonical_df, "history_flag"))
            .withColumn("lot_number", get_column(canonical_df, "lot_number"))
            .withColumn("vaccine_manufacturer", get_column(canonical_df, "vaccine_manufacturer"))
            .withColumn("immunization_dose", get_column(canonical_df, "immunization_dose"))
            .withColumn("site", get_column(canonical_df, "site"))
            .withColumn("route", get_column(canonical_df, "route"))
            .withColumn("expiration_date", get_column(canonical_df, "expiration_date"))
            .withColumn("cvx_code", get_column(canonical_df, "cvx_code"))
            .withColumn("cpt_code", get_column(canonical_df, "cpt_code"))
            .withColumn("immunization_given_by", get_column(canonical_df, "immunization_given_by"))
            .withColumn("immunization_status", get_column(canonical_df, "immunization_status"))
            .withColumn("vfc_status", get_column(canonical_df, "vfc_status"))
        ...
        # write data into delta lake `ecw_immunization` table
        immunization_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_immunization", opsgenie_api_key, file_details)
