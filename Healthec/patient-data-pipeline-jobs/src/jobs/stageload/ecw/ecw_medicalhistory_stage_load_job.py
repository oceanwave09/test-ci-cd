...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_medicalhistory_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_medicalhistory"))
        ...
        # add additional columns and prepare dataframe which match with ecw medicalhistory delta lake table structure
        medicalhistory_df = (
            ...
            .withColumn("past_history", get_column(canonical_df, "past_history"))
            .withColumn("history_icds", get_column(canonical_df, "history_icds"))
        ...
        # write data into delta lake `ecw_medicalhistory` table
        medicalhistory_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_medicalhistory", opsgenie_api_key, file_details)
