...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_encounter_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_encounter"))
        ...
        # add additional columns and prepare dataframe which match with ecw encounter delta lake table structure
        patient_df = (
            ...
            .withColumn("encounter_visit_type", get_column(canonical_df, "encounter_visit_type"))
            .withColumn("encounter_status", get_column(canonical_df, "encounter_status"))
            .withColumn("encounter_date", get_column(canonical_df, "encounter_date"))
            .withColumn("encounter_start_time", get_column(canonical_df, "encounter_start_time"))
            .withColumn("encounter_end_time", get_column(canonical_df, "encounter_end_time"))
            .withColumn("encounter_reason", get_column(canonical_df, "encounter_reason"))
            .withColumn("encounter_lock_status", get_column(canonical_df, "encounter_lock_status"))
            .withColumn("medication_reconciliation_flag", get_column(canonical_df, "medication_reconciliation_flag"))
            .withColumn("facility_name", get_column(canonical_df, "facility_name"))
            .withColumn("facility_id", get_column(canonical_df, "facility_id"))
            .withColumn("pos", get_column(canonical_df, "pos"))
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_encounter", opsgenie_api_key, file_details)
