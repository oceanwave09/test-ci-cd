...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_hospitalization_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ecw_hospitalization"))
        ...
        # add additional columns and prepare dataframe which match with ecw hospitalization delta lake table structure
        hospitalization_df = (
            ...
            .withColumn("reason", get_column(canonical_df, "reason"))
            .withColumn("hospitalization_date", get_column(canonical_df, "hospitalization_date"))
        ...
        # write data into delta lake `ecw_hospitalization` table
        hospitalization_df.write.format("delta").mode("append").save(delta_table_location)
        ...
        try:
            ...
            validation_result_page = run_validation(canonical_df, "ecw_hospitalization", opsgenie_api_key, file_details)
