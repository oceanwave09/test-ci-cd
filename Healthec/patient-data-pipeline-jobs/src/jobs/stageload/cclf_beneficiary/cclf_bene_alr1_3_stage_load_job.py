...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="cclf_bene_alr1_3_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "cclf_bene_alr1_3"),
        )
        ...
        # add additional columns and prepare dataframe which match with cclf_bene_alr1_3 delta lake table structure
        alr1_3_df = (
            ...
            .withColumn("rev_line_cnt", to_int_udf(get_column(data_df, "rev_line_cnt")))
        ...
        # write data into delta lake `cclf_bene_alr1_3` table
        alr1_3_df.write.format("delta").mode("append").save(delta_table_location)
