...
from utils.transformation import to_int
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="cclf_bene_alr1_9_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "cclf_bene_alr1_9"),
        )
        ...
        # add additional columns and prepare dataframe which match with cclf_bene_alr1_9 delta lake table structure
        alr1_9_df = (
            data_df.withColumn("row_id", get_column(data_df, "row_id"))
            .withColumn("bene_mbi_id", get_column(data_df, "bene_mbi_id"))
            .withColumn("adi_natrank", to_int_udf(get_column(data_df, "adi_natrank")))
            .withColumn("bene_lis_status", to_int_udf(get_column(data_df, "bene_lis_status")))
            .withColumn("bene_dual_status", to_int_udf(get_column(data_df, "bene_dual_status")))
            .withColumn("bene_psnyrs_lis_dual", to_int_udf(get_column(data_df, "bene_psnyrs_lis_dual")))
            .withColumn("bene_psnyrs", to_int_udf(get_column(data_df, "bene_psnyrs")))
        ...
        # write data into delta lake `cclf_bene_alr1_9` table
        alr1_9_df.write.format("delta").mode("append").save(delta_table_location)
