...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="cclf_bene_alr1_5_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "cclf_bene_alr1_5"),
        )
        ...
        # add additional columns and prepare dataframe which match with cclf_bene_alr1_5 delta lake table structure
        alr1_5_df = (
            ...
            .withColumn("plur_r05", to_int_udf(get_column(data_df, "plur_r05")))
            .withColumn("ab_r01", to_int_udf(get_column(data_df, "ab_r01")))
            .withColumn("hmo_r03", to_int_udf((get_column(data_df, "hmo_r03"))))
            .withColumn("no_us_r02", to_int_udf(get_column(data_df, "no_us_r02")))
            .withColumn("mdm_r04", to_int_udf(get_column(data_df, "mdm_r04")))
            .withColumn("nofnd_r06", to_int_udf(get_column(data_df, "nofnd_r06")))
        ...
        # write data into delta lake `cclf_bene_alr1_5` table
        alr1_5_df.write.format("delta").mode("append").save(delta_table_location)
