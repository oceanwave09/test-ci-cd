...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="cclf_bene_alr1_4_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "cclf_bene_alr1_4"),
        )
        ...
        # add additional columns and prepare dataframe which match with cclf_bene_alr1_4 delta lake table structure
        alr1_4_df = (
            ...
            .withColumn("master_id", get_column(data_df, "master_id"))
            .withColumn("npi_used", get_column(data_df, "npi_used"))
            .withColumn("pcs_count", to_int_udf(get_column(data_df, "pcs_count")))
        ...
        # write data into delta lake `cclf_bene_alr1_4` table
        alr1_4_df.write.format("delta").mode("append").save(delta_table_location)
