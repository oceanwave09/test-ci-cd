...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="cclf_bene_alr1_2_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "cclf_bene_alr1_2"),
        )
        ...
        # add additional columns and prepare dataframe which match with cclf_bene_alr1_2 delta lake table structure
        alr1_2_df = (
            ...
            .withColumn("b_em_line_cnt_t", to_int_udf(get_column(data_df, "b_em_line_cnt_t")))
        ...
        # write data into delta lake `cclf_bene_alr1_2` table
        alr1_2_df.write.format("delta").mode("append").save(delta_table_location)
