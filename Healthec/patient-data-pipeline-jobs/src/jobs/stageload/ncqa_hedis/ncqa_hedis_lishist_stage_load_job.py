...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ncqa_hedis_lishist_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ncqa_hedis_lishist"))
        ...
        # add additional columns and prepare dataframe which match with ncqa hedis lishist delta lake table structure
        lishist_df = (
            canonical_df.withColumn("row_id", get_column(canonical_df, "row_id"))
            .withColumn("record_type", get_column(canonical_df, "record_type"))
            .withColumn("beneficiary_id", get_column(canonical_df, "beneficiary_id"))
            .withColumn("low_income_period_start_date", get_column(canonical_df, "low_income_period_start_date"))
            .withColumn("low_income_period_end_date", get_column(canonical_df, "low_income_period_end_date"))
        ...
        lishist_df.write.format("delta").mode("append").save(delta_table_location)
