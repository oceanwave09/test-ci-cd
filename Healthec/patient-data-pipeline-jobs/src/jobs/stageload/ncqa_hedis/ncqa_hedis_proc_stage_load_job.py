...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ncqa_hedis_proc_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        # add additional columns and prepare dataframe which match with ncqa hedis proc delta lake table structure
        proc_df = (
            ...
            .withColumn(
                "procedure_code",
                get_column(canonical_df, "procedure_code"),
            )
            .withColumn(
                "code_flag",
            ...
            .withColumn(
                "service_status",
                get_column(canonical_df, "service_status"),
            )
            .withColumn(
                "attribute",
                get_column(canonical_df, "attribute"),
            )
            .withColumn(
                "batch_id",
        ...
        proc_df.write.format("delta").mode("append").save(delta_table_location)
