...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ncqa_hedis_provider_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        # add additional columns and prepare dataframe which match with ncqa hedis provider delta lake table structure
        provider_df = (
            ...
            .withColumn("pcp", get_column(canonical_df, "pcp"))
            .withColumn("obgyn", get_column(canonical_df, "obgyn"))
            .withColumn(
                "mh_provider",
                get_column(canonical_df, "mh_provider"),
            )
            .withColumn(
                "eye_care_provider",
                get_column(canonical_df, "eye_care_provider"),
            )
            .withColumn(
                "dentist",
                get_column(canonical_df, "dentist"),
            )
            .withColumn(
                "nephrologist",
                get_column(canonical_df, "nephrologist"),
            )
            .withColumn(
                "anesthesiologist",
                get_column(canonical_df, "anesthesiologist"),
            )
            .withColumn(
                "npr_provider",
                get_column(canonical_df, "npr_provider"),
            )
            .withColumn(
                "pas_provider",
                get_column(canonical_df, "pas_provider"),
            )
            .withColumn(
                "provider_prescribing_privileges",
                get_column(
                    canonical_df,
                    "provider_prescribing_privileges",
                ),
            ...
            .withColumn(
                "clinical_pharmacist",
                get_column(canonical_df, "clinical_pharmacist"),
            )
            .withColumn(
                "hospital",
                get_column(canonical_df, "hospital"),
            )
            .withColumn("snf", get_column(canonical_df, "snf"))
            .withColumn(
                "surgeon",
                get_column(canonical_df, "surgeon"),
            )
            .withColumn(
                "registered_nurse",
                get_column(canonical_df, "registered_nurse"),
            )
            .withColumn(
                "batch_id",
