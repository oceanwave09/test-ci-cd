...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ncqa_hedis_pharm_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        # add additional columns and prepare dataframe which match with ncqa hedis pharm delta lake table structure
        pharm_df = (
            ...
            .withColumn(
                "days_supply",
                get_column(canonical_df, "days_supply"),
            )
            .withColumn(
                "service_date",
            ...
            .withColumn(
                "ndc_drug_code",
                get_column(canonical_df, "ndc_drug_code"),
            )
            .withColumn(
                "claim_status",
            ...
            .withColumn(
                "quantity_dispensed",
                get_column(canonical_df, "quantity_dispensed"),
            )
            .withColumn(
                "supplemental_data",
            ...
            .withColumn(
                "provider_npi",
                get_column(canonical_df, "provider_npi"),
            )
            .withColumn(
                "pharmacy_npi",
                get_column(canonical_df, "pharmacy_npi"),
            )
            .withColumn(
                "batch_id",
        ...
        pharm_df.write.format("delta").mode("append").save(delta_table_location)
