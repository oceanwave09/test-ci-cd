...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ncqa_hedis_diag_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ncqa_hedis_diag"))
        ...
        # add additional columns and prepare dataframe which match with ncqa hedis diag delta lake table structure
        diag_df = (
            canonical_df.withColumn("row_id", get_column(canonical_df, "row_id"))
            .withColumn("member_id", get_column(canonical_df, "member_id"))
            .withColumn("diagnosis_code", get_column(canonical_df, "diagnosis_code"))
            .withColumn("diagnosis_flag", get_column(canonical_df, "diagnosis_flag"))
            .withColumn("start_date", get_column(canonical_df, "start_date"))
            .withColumn("end_date", get_column(canonical_df, "end_date"))
            .withColumn("attribute", get_column(canonical_df, "attribute"))
        ...
        diag_df.write.format("delta").mode("append").save(delta_table_location)
