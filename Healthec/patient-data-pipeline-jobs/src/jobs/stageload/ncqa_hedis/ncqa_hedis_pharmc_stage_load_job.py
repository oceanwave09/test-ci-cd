...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ncqa_hedis_pharmc_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        # add additional columns and prepare dataframe which match with ncqa hedis pharmc delta lake table structure
        pharm_c_df = (
            ...
            .withColumn(
                "ordered_date",
                get_column(canonical_df, "ordered_date"),
            )
            .withColumn(
                "start_date",
                get_column(canonical_df, "start_date"),
            )
            .withColumn(
                "drug_code",
                get_column(canonical_df, "drug_code"),
            )
            .withColumn(
                "code_flag",
                get_column(canonical_df, "code_flag"),
            )
            .withColumn(
                "frequency",
                get_column(canonical_df, "frequency"),
            )
            .withColumn(
                "dispensed_date",
                get_column(canonical_df, "dispensed_date"),
            )
            .withColumn(
                "end_date",
                get_column(canonical_df, "end_date"),
            )
            .withColumn(
                "active_or_inactive_flag",
                get_column(canonical_df, "active_or_inactive_flag"),
            )
            .withColumn(
                "year_of_immunization",
                get_column(canonical_df, "year_of_immunization"),
            )
            .withColumn(
                "quantity",
                get_column(canonical_df, "quantity"),
            )
            .withColumn(
                "batch_id",
        ...
        pharm_c_df.write.format("delta").mode("append").save(delta_table_location)
