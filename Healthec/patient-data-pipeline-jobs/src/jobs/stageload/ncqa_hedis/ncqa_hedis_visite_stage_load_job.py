...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ncqa_hedis_visite_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ncqa_hedis_visite"),
        )
        ...
        canonical_df = canonical_df.withColumnRenamed("status", "encounter_status")

        # add additional columns and prepare dataframe which match with ncqa hedis visite delta lake table structure
        visit_e_df = (
            ...
            .withColumn(
                "service_date",
                get_column(canonical_df, "service_date"),
            )
            .withColumn(
                "activity_type",
                get_column(canonical_df, "activity_type"),
            )
            .withColumn(
                "code_flag",
            ...
            .withColumn(
                "encounter_status",
                get_column(canonical_df, "encounter_status"),
            )
            .withColumn(
                "provider_id",
            ...
            .withColumn(
                "diagnosis_code",
                get_column(canonical_df, "diagnosis_code"),
            )
            .withColumn(
                "diagnosis_flag",
                get_column(canonical_df, "diagnosis_flag"),
            )
            .withColumn(
                "batch_id",
        ...
        visit_e_df.write.format("delta").mode("append").save(delta_table_location)
