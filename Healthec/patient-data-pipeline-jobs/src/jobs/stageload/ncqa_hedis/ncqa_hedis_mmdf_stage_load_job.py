...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ncqa_hedis_mmdf_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ncqa_hedis_mmdf"))
        ...
        # add additional columns and prepare dataframe which match with ncqa hedis mmdf delta lake table structure
        mmdf_df = (
            canonical_df.withColumn("row_id", get_column(canonical_df, "row_id"))
            .withColumn("run_date", get_column(canonical_df, "run_date"))
            .withColumn("payment_date", get_column(canonical_df, "payment_date"))
            .withColumn("beneficiary_id", get_column(canonical_df, "beneficiary_id"))
            .withColumn("hospice", get_column(canonical_df, "hospice"))
            .withColumn("long_term_institutionalized_flag", get_column(canonical_df, "long_term_institutionalized_flag"))
            .withColumn(
                "original_reason_for_entitlement_code", get_column(canonical_df, "original_reason_for_entitlement_code")
            )
            .withColumn("unknown_field_1", get_column(canonical_df, "unknown_field_1"))
        ...
        mmdf_df.write.format("delta").mode("append").save(delta_table_location)
