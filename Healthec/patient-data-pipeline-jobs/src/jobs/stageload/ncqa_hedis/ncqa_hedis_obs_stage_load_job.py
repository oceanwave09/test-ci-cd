...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ncqa_hedis_obs_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        canonical_df = canonical_df.withColumnRenamed("status", "observation_status")

        # add additional columns and prepare dataframe which match with ncqa hedis obs delta lake table structure
        obs_df = (
            ...
            .withColumn(
                "observation_date",
                get_column(canonical_df, "observation_date"),
            )
            .withColumn("test", get_column(canonical_df, "test"))
            .withColumn(
                "test_code_flag",
                get_column(canonical_df, "test_code_flag"),
            )
            ...
            .withColumn(
                "observation_status",
                get_column(canonical_df, "observation_status"),
            )
            .withColumn(
                "result_value_flag",
                get_column(canonical_df, "result_value_flag"),
            )
            .withColumn("type", get_column(canonical_df, "type"))
            .withColumn(
                "batch_id",
        ...
        obs_df.write.format("delta").mode("append").save(delta_table_location)
