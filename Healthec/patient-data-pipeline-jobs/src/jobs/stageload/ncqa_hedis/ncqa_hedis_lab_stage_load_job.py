...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ncqa_hedis_lab_stage_load_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get("DELTA_TABLE_NAME", config.get("delta_table_name", "ncqa_hedis_lab"))
        ...
        # add additional columns and prepare dataframe which match with ncqa hedis lab delta lake table structure
        lab_df = (
            ...
            .withColumn("loinc_code", get_column(canonical_df, "loinc_code"))
            .withColumn("value", get_column(canonical_df, "value"))
            .withColumn("date_of_service", get_column(canonical_df, "date_of_service"))
            .withColumn("supplemental_data", get_column(canonical_df, "supplemental_data"))
