...
#     data_dict["group_npi"] = row_dict.get("rendering_facility_npi", "")
#     data_dict["source_id"] = row_dict.get("rendering_facility_internal_id", "")
#     data_dict["name"] = row_dict.get("rendering_facility_name", "")
...
def _transform_encounter(row_dict: dict, transformer: FHIRTransformer) -> dict:
    ...
    # data_dict["practitioner_id"] = row_dict.get("attending_provider_internal_id", "")
...
def _transform_claim(row_dict: dict, transformer: FHIRTransformer) -> dict:
    ...
    for i, line_entry in enumerate(row_dict.get("service_lines", [])):
        ...
        if line_entry.get("service_start_date", ""):
            service_line["service_period_start"] = line_entry.get("service_start_date", "")
        if line_entry.get("service_end_date", ""):
            ...
            product_or_service.append(
                {
                    ...
                    "display": line_entry.get("cpt_code_desc", ""),
            ...
            product_or_service.append(
                {
                    ...
                    "display": line_entry.get("hcpcs_code_desc", ""),
                    "system": HCPCS_CODE_SYSTEM,
                }
            )
        service_line["product_or_service"] = product_or_service
        svc_modifiers = []
        if line_entry.get("service_modifier_1", ""):
            svc_modifiers.append({"modifier_code": line_entry.get("service_modifier_1", "")})
        if line_entry.get("service_modifier_2", ""):
            svc_modifiers.append({"modifier_code": line_entry.get("service_modifier_2", "")})
        if line_entry.get("service_modifier_3", ""):
            svc_modifiers.append({"modifier_code": line_entry.get("service_modifier_3", "")})
        if line_entry.get("service_modifier_4", ""):
            svc_modifiers.append({"modifier_code": line_entry.get("service_modifier_4", "")})
        service_line["svc_modifiers"] = svc_modifiers
        if line_entry.get("service_units"):
            service_line["quantity_value"] = line_entry.get("service_units")
...
def _transform_claim_response(row_dict: dict, transformer: FHIRTransformer) -> dict:
    ...
    for i, line_entry in enumerate(row_dict.get("service_lines", "")):
        ...
        adjudications = []
        if line_entry.get("service_line_charges", ""):
            adjudications.append(_get_adjudication(line_entry.get("service_line_charges", ""), "submitted"))
        if line_entry.get("line_amount_paid", ""):
            adjudications.append(_get_adjudication(line_entry.get("line_amount_paid", ""), "benefit"))
        if line_entry.get("line_deduct_amount", ""):
            adjudications.append(_get_adjudication(line_entry.get("line_deduct_amount", ""), "deductible"))
        if line_entry.get("line_copay_amount", ""):
            adjudications.append(_get_adjudication(line_entry.get("line_copay_amount", ""), "copay"))
...
def render_resources(row: Row, transformer: FHIRTransformer) -> Row:
    ...
    # if row_dict.get("rendering_facility_npi"):
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="medical_claim_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        data_df = (
            ...
            .withColumn(
                "referring_npi",
                f.expr(f"aes_decrypt(unhex(referring_npi) ,'{pipeline_data_key}', 'ECB', 'PKCS')").cast("string"),
            )
            .withColumn(
                "referring_last_name",
                f.expr(f"aes_decrypt(unhex(referring_last_name) ,'{pipeline_data_key}', 'ECB', 'PKCS')").cast("string"),
            )
            .withColumn(
                "referring_first_name",
                f.expr(f"aes_decrypt(unhex(referring_first_name) ,'{pipeline_data_key}', 'ECB', 'PKCS')").cast("string"),
            )
            # .withColumn(
            #     "rendering_facility_npi",
            #     f.expr(f"aes_decrypt(unhex(rendering_facility_npi) ,'{pipeline_data_key}', 'ECB', 'PKCS')").cast(
            #         "string"
            #     ),
            # )
            .withColumn(
        ...
        prov_df = data_df.filter(f.col("rendering_facility_name") == "")
        if not prov_df.isEmpty():
            exit_with_error(log, "rendering facility name must be not-null values.")
        ...
        data_df = (
            ...
            .withColumn("service_units", transform_to_float(f.col("service_units")))
            ...
            .withColumn("service_line_paid_date", transform_date_time(f.col("service_line_paid_date")))
            .withColumn("line_payment_level", transform_to_float(f.col("line_payment_level")))
            .withColumn("line_deduct_amount", transform_to_float(f.col("line_deduct_amount")))
            .withColumn("line_copay_amount", transform_to_float(f.col("line_copay_amount")))
            .withColumn("line_coinsurance_amount", transform_to_float(f.col("line_coinsurance_amount")))
            .withColumn("line_allowed_amount", transform_to_float(f.col("line_allowed_amount")))
            .withColumn("line_discount_amount", transform_to_float(f.col("line_discount_amount")))
            .withColumn("line_patient_paid_amount", transform_to_float(f.col("line_patient_paid_amount")))
            .withColumn("line_other_payer_paid", transform_to_float(f.col("line_other_payer_paid")))
            .withColumn("medicare_paid_amount", transform_to_float(f.col("medicare_paid_amount")))
            .withColumn("line_allowed_units", transform_to_float(f.col("line_allowed_units")))
        ...
        line_proc_df = data_df.withColumnRenamed("principal_procedure_code", "procedure_code").select(
        ...
        group_by_cols = (
            ...
            "discharge_status",
            "place_of_service",
            "rendering_facility_internal_id",
            "rendering_facility_name",
        ...
        claim_data_df = line_data_df.groupBy(*group_by_cols).agg(
            f.collect_list(
                f.struct(
                    ...
                    f.col("cpt_code_desc"),
                    f.col("hcpcs_code"),
                    f.col("hcpcs_code_desc"),
                    f.col("service_modifier_1"),
                    f.col("service_modifier_2"),
                    f.col("service_modifier_3"),
                    f.col("service_modifier_4"),
                    f.col("service_units"),
        ...
        claim_data_df = (
            ...
            .withColumn(
                ...
                f.when((f.col("rendering_facility_name") != ""), f.expr("uuid()")).otherwise(f.lit("")),
            )
