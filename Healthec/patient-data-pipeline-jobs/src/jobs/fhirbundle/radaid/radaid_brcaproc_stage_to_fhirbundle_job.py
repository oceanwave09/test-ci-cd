...
def _transform_procedure(row_dict: dict, transformer: FHIRTransformer) -> Row:
    ...
    if row_dict.get("proc_name", ""):
        data_dict["codes"].append(
            {
                "code": fetch_code_using_name(PROCEDURE_NAME_DICT, row_dict.get("proc_name", "")),
    ...
    data_dict["outcome_display"] = PATHOLOGY_RESULT_DICT.get(row_dict.get("proc_result", ""), "")
    data_dict["outcome_system"] = BRCA_PROC_OUTCOME_CODE_SYSTEM
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="radaid_brcaproc_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "radaid_brcaproc"),
        )
