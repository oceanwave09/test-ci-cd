...
def _transform_procedure(row_dict: dict, transformer: FHIRTransformer) -> Row:
    ...
    if row_dict.get("diag_name", ""):
        data_dict["codes"].append(
            {
                "code": fetch_code_using_name(CXCA_DIAGNOSIS_METHOD_DICT, row_dict.get("diag_name", "")),
                "code_display": row_dict.get("diag_name"),
                "code_system": CXCA_DIAG_CODE_SYSTEM,
            }
        )
    ...
    data_dict["outcome_display"] = CXCA_PATHOLOGY_RESULT_DICT.get(row_dict.get("diag_result", ""), "")
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="radaid_cxcadiag_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "radaid_cxcadiag"),
        )
        ...
        # Filter rows where either `diag_code` is not null and not empty, or `diag_name` is not null and not empty
