...
from utils.constants import (
    CXCA_DIAG_OUTCOME_CODE_SYSTEM,
    CXCA_PATHOLOGY_RESULT_DICT,
    CXCA_PROCEDURE_CODE_SYSTEM,
            ...
            "identifier": [
                {
                    ...
                    "system": "http://healthec.com/identifier/patient/mrn",
                    "value": "$mrn",
                    "assigner": {
...
def _transform_procedure(row_dict: dict, transformer: FHIRTransformer) -> Row:
    ...
    data_dict["codes"] = []
    if row_dict.get("proc_name", ""):
        data_dict["codes"].append(
            {
                "code": fetch_code_using_name(CXCA_PROCEDURE_NAME_DICT, row_dict.get("proc_name", "")),
                "code_display": row_dict.get("proc_name"),
                "code_system": CXCA_PROCEDURE_CODE_SYSTEM,
            }
        )
    if row_dict.get("proc_code", ""):
        data_dict["codes"].append(
            {
                "code": row_dict.get("proc_code", ""),
                "code_display": row_dict.get("display", ""),
                "code_system": row_dict.get("system", ""),
            }
        )
    data_dict["outcome_code"] = row_dict.get("proc_result", "")
    data_dict["outcome_display"] = CXCA_PATHOLOGY_RESULT_DICT.get(row_dict.get("proc_result", ""), "")
    data_dict["outcome_system"] = CXCA_DIAG_OUTCOME_CODE_SYSTEM
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="radaid_cxcaproc_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "radaid_cxcaproc"),
        )
        ...
        # Filter rows where either `proc_code` is not null and not empty, or `proc_name` is not null and not empty
        result_df = data_df.filter((f.col("proc_code") != "") | (f.col("proc_name") != ""))
        ...
        # Join result_df with master_value_df using a left join on the `proc_code` column
        result_df = result_df.join(broadcast(master_value_df), result_df["proc_code"] == master_value_df["code"], "left")

        # apply transformations
        result_df = (
            result_df.withColumn("procedure_internal_id", f.expr("uuid()"))
            .withColumn("procedure_performed_date_time", transform_date_time(f.col("proc_date")))
