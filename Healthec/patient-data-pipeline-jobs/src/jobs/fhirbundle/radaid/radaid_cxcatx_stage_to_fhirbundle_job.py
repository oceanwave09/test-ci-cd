...
def _transform_procedure(row_dict: dict, transformer: FHIRTransformer) -> Row:
    ...
    data_dict["codes"] = []
    if row_dict.get("tx_name", ""):
        data_dict["codes"].append(
            {
                "code": fetch_code_using_name(TREATMENT_DICT, row_dict.get("tx_name", "")),
                "code_display": row_dict.get("tx_name"),
                "code_system": CXCA_PROCEDURE_CODE_SYSTEM,
            }
        )
    if row_dict.get("tx_code", ""):
        data_dict["codes"].append(
            {
                "code": row_dict.get("tx_code", ""),
                "code_display": row_dict.get("display", ""),
                "code_system": row_dict.get("system", ""),
            }
        )
    data_dict["outcome_code"] = row_dict.get("stage", "")
    data_dict["outcome_display"] = TNM_STAGE_DICT.get(row_dict.get("stage", ""), "")
    data_dict["outcome_system"] = TX_OUTCOME_CODE_SYSTEM
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="radaid_cxcatx_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "radaid_cxcatx"),
        )
        ...
        # Filter rows where either `tx_code` is not null and not empty, or `tx_name` is not null and not empty
        result_df = data_df.filter((f.col("tx_code") != "") | (f.col("tx_name") != ""))
        ...
        # Join result_df with master_value_df using a left join on the `tx_code` column
        result_df = result_df.join(broadcast(master_value_df), result_df["tx_code"] == master_value_df["code"], "left")

        # apply transformations
        result_df = (
            result_df.withColumn("procedure_internal_id", f.expr("uuid()"))
            .withColumn("procedure_performed_date_time", transform_date_time(f.col("tx_date")))
