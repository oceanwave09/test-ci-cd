...
COMM_JINJA_TEMPLATE = "communication.j2"
...
def _transform_communication(row_dict: dict, transformer: FHIRTransformer) -> Row:
    data_dict = {}
    data_dict["internal_id"] = row_dict.get("communication_internal_id", "")
    data_dict["patient_id"] = row_dict.get("patient_id", "")
    data_dict["topic_text"] = row_dict.get("outreach_type", "")
    data_dict["status"] = row_dict.get("pn_contact", "")
    data_dict["sent_date"] = row_dict.get("communication_sent_date")
    data_dict["note_text"] = row_dict.get("pn_barrier", "")
    data_dict["status_reason_code"] = row_dict.get("outcome", "")

    return _transform_resource(
        data_dict=data_dict,
        template=COMM_JINJA_TEMPLATE,
        resource_type=ResourceType.Communication.value,
...
def render_resources(row: Row, transformer: FHIRTransformer) -> Row:
    ...
    resources = []
    # render resources
    resources.append(_transform_communication(row_dict, transformer))
    return Row(
        **{
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="radaid_ptoutreach_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "radaid_ptoutreach"),
        )
        ...
        data_df = data_df.join(match_pat_df, on="mrn", how="inner")

        # apply transformations
        data_df = (
            data_df.withColumn("communication_internal_id", f.expr("uuid()"))
            .withColumn("communication_sent_date", transform_date_time(f.col("pn_contact_date")))
