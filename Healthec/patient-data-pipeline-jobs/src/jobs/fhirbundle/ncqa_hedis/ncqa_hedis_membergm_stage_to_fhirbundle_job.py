...
from utils.constants import NCQA_MEMBER_ID_SRC_SYSTEM
    ...
    df = df.select(
        ...
        f.map_from_arrays(
            ...
            f.array(
                ...
                f.lit("member-gm"),
                f.lit("https://healthec.com/identifiers/file"),
                f.col("member_id"),
                f.lit(NCQA_MEMBER_ID_SRC_SYSTEM),
                f.col("src_organization_id"),
                f.col("gender"),
                f.col("date_of_birth"),
                f.col("member_first_name"),
                f.col("member_last_name"),
                f.col("member_middle_initial"),
                f.col("mailing_address_1"),
                f.col("mailing_address_2"),
                f.col("city"),
                f.col("state"),
                f.col("zip"),
                f.col("telephone_number"),
                f.col("spoken_language"),
                f.col("race"),
                f.col("ethnicity"),
            ),
        ).alias("patient_rsc"),
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ncqa_hedis_membergm_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ncqa_hedis_membergm"),
        )
        ...
        member_first_name = f.concat_ws(
            "-",
            f.when(f.col("member_first_name") != "", f.col("member_first_name")).otherwise(f.lit("FirstName")),
            f.col("member_id"),
        )

        member_last_name = f.concat_ws(
            "-",
            f.when(f.col("member_last_name") != "", f.col("member_last_name")).otherwise(f.lit("LastName")),
            f.col("member_id"),
        )
        ...
        data_df = (
            ...
            .withColumn("date_of_birth", transform_date(f.col("date_of_birth")))
            .withColumn("gender", patient_gender_col)
            .withColumn("member_first_name", member_first_name)
            .withColumn("member_last_name", member_last_name)
