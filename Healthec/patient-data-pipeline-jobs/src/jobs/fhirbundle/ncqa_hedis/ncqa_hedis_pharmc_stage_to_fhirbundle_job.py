...
MED_STATE_JINJA_TEMPLATE = "medication_statement.j2"
...
def fhir_mapper_df(df: DataFrame) -> DataFrame:
    df = df.select(
        "bundle_id",
        f.create_map(f.lit("dosages"), f.col("medication_statement_dosages")).alias("medi_stat_dosages"),
        f.map_from_arrays(
            f.array(
                f.lit("internal_id"),
                f.lit("member_id"),
        ...
        f.map_from_arrays(
            ...
            f.array(
                f.col("medication_statement_internal_id"),
                f.col("patient_internal_id"),
                f.lit("pharm-c"),
                ...
                f.col("drug_code"),
                f.col("code_flag"),
                f.col("active_or_inactive_flag"),
            ),
        ).alias("medi_statement_rsc"),
    )
    return df
...
def render_resources(row: Row, transformer: FHIRTransformer) -> Row:
    ...
    # adding dosage
    row_dict.get("medi_statement_rsc").update(row_dict.get("medi_stat_dosages"))
    resources.append(
        _transform_resource(
            data_dict=row_dict.get("medi_statement_rsc"),
            resource_type=ResourceType.MedicationStatement.value,
            template=MED_STATE_JINJA_TEMPLATE,
            transformer=transformer,
        )
    )
    return Row(
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ncqa_hedis_pharmc_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ncqa_hedis_pharmc"),
        )
        ...
        # transformation
        medi_statement_status_col = (
            f.when(f.col("active_or_inactive_flag") == "1", "active")
            .when(f.col("active_or_inactive_flag") == "2", "in-active")
            .otherwise("unknown")
        )

        medi_statement_dosages = f.array(
                f.struct(
        ...
        data_df = (
            ...
            .withColumn("code_flag", transform_code_system_pharm(f.col("code_flag")))
            .withColumn("active_or_inactive_flag", medi_statement_status_col)
            .withColumn("medication_statement_dosages", medi_statement_dosages)
