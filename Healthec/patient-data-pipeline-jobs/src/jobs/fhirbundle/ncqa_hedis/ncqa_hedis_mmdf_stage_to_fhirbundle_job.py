...
from utils.constants import HOSPICE_URL, LONG_TERM_INSTITUTE_CARE_URL, NCQA_MEMBER_ID_SRC_SYSTEM
...
def fhir_mapper_df(df: DataFrame) -> DataFrame:
    df = df.select(
        "bundle_id",
        f.create_map(f.lit("extensions"), f.col("encnt_extension")).alias("encounter_extension"),
        f.map_from_arrays(
            f.array(
                f.lit("internal_id"),
                f.lit("member_id"),
        ...
        f.map_from_arrays(
            f.array(f.lit("internal_id"), f.lit("patient_id"), f.lit("period_start_date"), f.lit("period_end_date")),
            f.array(
...
# def _transform_coverage(row: Row, transformer: FHIRTransformer, metadata: dict) -> Row:
...
#     data_dict["member_id"] = row_dict.get("coverage_member_id", "")
#     data_dict["member_system"] = NCQA_MEMBER_ID_SRC_SYSTEM
#     data_dict["assigner_organization_id"] = metadata.get("src_organization_id", "")

#     # parse hospice_flag and long_term_institutionalized_flag data
#     data_dict["extensions"] = []
#     if row_dict.get("coverage_hospice_flag") == "Y":
#         data_dict["extensions"].append(
#             {"url": HOSPICE_URL, "value_extensions": [{"url": HOSPICE_URL + "#value", "value_boolean": "true"}]}
#         )
#     if row_dict.get("coverage_long_term_institutionalized_flag") == "Y":
#         data_dict["extensions"].append(
#             {
#                 "url": LONG_TERM_INSTITUTE_CARE_URL,
#                 "value_extensions": [{"url": LONG_TERM_INSTITUTE_CARE_URL + "#value", "value_boolean": "true"}],
#             }
#         )
#     if row_dict.get("coverage_run_date"):
#         run_date = parse_date(str(row_dict.get("coverage_run_date", "")))
#         data_dict["extensions"].append(
#             {"url": RUN_DATE_URL, "value_extensions": [{"url": RUN_DATE_URL + "#value", "value_date": run_date}]}
#         )
#     resource_dict = {}
#     resource_dict["row_id"] = row_dict.get("row_id")
#     resource_dict["resources"] = [transformer.render_resource(ResourceType.Coverage.value, data_dict)]

#     return Row(**resource_dict)


# def render_coverage(spark: SparkSession, transformer: FHIRTransformer, metadata: dict, seq: int = 1, pat_seq: int = 1):
...
#             )
#             .withColumn("coverage_run_date", f.col("run_date"))
#             .withColumn("coverage_member_id", f.col("beneficiary_id"))
#             .withColumn("coverage_hospice_flag", f.col("hospice"))
#             .withColumn("coverage_long_term_institutionalized_flag", f.col("long_term_institutionalized_flag"))
#             .select("row_id", df.colRegex("`coverage_.*`"))
#         )
#         cov_rdd = cov_df.rdd.map(lambda row: _transform_coverage(row, transformer, metadata))
...
def render_resources(row: Row, transformer: FHIRTransformer) -> Row:
    ...
    # adding extension
    row_dict.get("encounter_rsc").update(row_dict.get("encounter_extension"))
    resources.append(
        _transform_resource(
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ncqa_hedis_mmdf_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ncqa_hedis_mmdf"),
        )
        ...
        # filter rows with hospice = "Y" or long_term_institutionalized_flag = "Y"
        data_df = data_df.filter((f.col("hospice") == "Y") | (f.col("long_term_institutionalized_flag") == "Y"))
        ...
        # transformation
        encnt_extension_col = f.array(
            f.when(
                f.col("hospice") == "Y",
                f.create_map(f.lit("url"), f.lit(HOSPICE_URL), f.lit("value_boolean"), f.lit("true")),
            ),
            f.when(
                f.col("long_term_institutionalized_flag") == "Y",
                f.create_map(f.lit("url"), f.lit(LONG_TERM_INSTITUTE_CARE_URL), f.lit("value_boolean"), f.lit("true")),
            ),
        )
        ...
        data_df = (
            ...
            .withColumn("encnt_extension", encnt_extension_col)
            .withColumn("run_date", transform_date_time(f.col("run_date")))
