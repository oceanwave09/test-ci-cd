...
def render_resources(row: Row, transformer: FHIRTransformer) -> Row:
    ...
    if row_dict.get("condition_rsc", {}).get("diagnosis_code"):
        resources.append(
            _transform_resource(
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ncqa_hedis_visite_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        # transformations
        encnt_status_col = (
            f.when(f.col("encounter_status") == "1", "planned")
            .when(f.col("encounter_status") == "2", "finished")
            .otherwise("unknown")
        )
        ...
        data_df = (
            ...
            .withColumn("encounter_status", encnt_status_col)
            .withColumn("diagnosis_flag", transform_code_system(f.col("diagnosis_flag")))
