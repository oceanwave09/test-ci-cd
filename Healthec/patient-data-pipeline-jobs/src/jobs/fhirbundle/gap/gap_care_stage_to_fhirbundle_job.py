...
from utils.constants import CPT_CODE_SYSTEM, PATIENT_MRN_SYSTEM
...
def _transform_procedure(row_dict: dict, transformer: FHIRTransformer) -> dict:
    ...
    if str(row_dict.get("dictionary_name", "")).lower() == "cpt":
        data_dict["code_system"] = CPT_CODE_SYSTEM
    data_dict["code_text"] = row_dict.get("name", "")
    data_dict["performed_date_time"] = row_dict.get("performed_date_time")
    if row_dict.get("visit_reason_code"):
        data_dict["reasons"] = [
            {
                "code": row_dict.get("visit_reason_code", ""),
                "code_display": row_dict.get("visit_reason_name", ""),
                "code_text": row_dict.get("visit_reason_dictionary", ""),
            }
        ]
    if row_dict.get("modifier_code"):
        data_dict["modifiers"] = [
            {
                "code": row_dict.get("modifier_code", ""),
                "code_display": row_dict.get("value", ""),
                "code_text": row_dict.get("modifier_dictionary_name", ""),
            }
        ]
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="gap_care_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "gap_care"),
        )
        ...
        # transformations
        pat_mihin_id = f.when(f.col("patient_id") != "None", f.col("patient_id"))
        practitioner_name = f.col("provider_name")
        practitioner_first_name = (
            f.when(f.size(f.split(f.col("provider_name"), " ")) > 1, f.split(f.col("provider_name"), " ")[0])
            .when(f.size(f.split(f.col("provider_name"), ",")) > 1, f.split(f.col("provider_name"), ",")[0])
            .otherwise(f.col("provider_name"))
        )

        practitioner_last_name = (
            f.when(f.size(f.split(f.col("provider_name"), " ")) > 1, f.split(f.col("provider_name"), " ")[1])
            .when(f.size(f.split(f.col("provider_name"), ",")) > 1, f.split(f.col("provider_name"), ",")[1])
            .otherwise("")
        )
        ...
        data_df = (
            ...
            .withColumn("performed_date_time", transform_date_time(f.col("date_performed")))
            .withColumn("patient_id", pat_mihin_id)
            .withColumn("practitioner_name", practitioner_name)
