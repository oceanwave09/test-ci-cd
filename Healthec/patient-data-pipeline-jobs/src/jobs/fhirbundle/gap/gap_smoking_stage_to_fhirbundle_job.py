...
from utils.constants import SNOMED_CODE_SYSTEM, PATIENT_MRN_SYSTEM
...
def _get_smoking_snomed(value: str):
    if value == ["Current every day smoker", "Every Day"]:
        return "449868002"
    elif value == ["Current some day smoker", "Some Days"]:
        return "428041000124106"
    elif value == "Smoker, current status unknown":
        return "77176002"
    elif value == "Heavy tobacco smoker":
        return "428071000124103"
    elif value == "Light tobacco smoker":
        return "428061000124105"
    elif value == ["Former smoker", "Former"]:
        return "8517006"
    elif value in [
        "Never",
        "Never smoker",
        ...
        return "266919005"
    elif value == "Unknown if ever smoked":
        return "266927001"
    else:
        return ""
    ...
    df = df.select(
        ...
        f.map_from_arrays(
            f.array(
                ...
                f.lit("loinc_code"),
                f.lit("loinc_code_display"),
                f.lit("loinc_code_text"),
                f.lit("loinc_code_system"),
            ...
            f.array(
                ...
                f.col("encounter_internal_id"),
                f.lit("final"),
                f.lit("social-history"),
                f.lit("Social History"),
                f.lit("Social History"),
                f.lit("http://terminology.hl7.org/CodeSystem/observation-category"),
                f.lit("72166-2"),
                f.lit("Tobacco smoking status NHIS"),
                f.lit("Tobacco smoking status NHIS"),
                f.lit("http://loinc.org"),
                f.col("last_confirmed_date"),
                f.col("obsr_value_code"),
                f.lit(SNOMED_CODE_SYSTEM),
                f.col("smoking_status_name"),
            ),
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="gap_smoking_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "gap_smoking"),
        )
        ...
        data_df = data_df.filter(
            (f.col("last_confirmed_date") != "")
            & (f.col("last_confirmed_date") != "99991231")
            # & (f.col("remove_specified_smoking_status_flag") != "1")
            & (f.col("smoking_status_name") != "")
        )
        ...
        get_smoking_snomed_udf = f.udf(_get_smoking_snomed)
        ...
        data_df = (
            ...
            .withColumn("last_confirmed_date", transform_date_time(f.col("last_confirmed_date")))
            .withColumn("obsr_value_code", get_smoking_snomed_udf(f.col("smoking_status_name")))
