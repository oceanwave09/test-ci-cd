...
def _transform_lab(row_dict: dict, transformer: FHIRTransformer) -> str:
    data_dict = {}
    ...
    if to_float(row_dict.get("value", "")):
        data_dict["quantity_value"] = to_float(row_dict.get("value"))
    if to_float(row_dict.get("low_referance_range", "")) is not None:
        data_dict["reference_range_low_value"] = to_float(row_dict.get("low_referance_range"))
    if to_float(row_dict.get("high_referance_range", "")):
        data_dict["reference_range_high_value"] = to_float(row_dict.get("high_referance_range"))
...
def render_resources(row: Row, transformer: FHIRTransformer) -> Row:
    ...
    resources.append(_transform_lab(row_dict, transformer))
    return Row(
        **{
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="gap_lab_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
