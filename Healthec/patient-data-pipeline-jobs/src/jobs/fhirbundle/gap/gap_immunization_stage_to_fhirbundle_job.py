...
from utils.constants import CVX_CODE_SYSTEM, PATIENT_MRN_SYSTEM
...
IMU_JINJA_TEMPLATE = "immunization.j2"
...
def _transform_immunization(row_dict: dict, transformer: FHIRTransformer) -> str:
    data_dict = {}
    ...
    data_dict["vaccine_code"] = row_dict.get("code", "")
    if str(row_dict.get("dictionary_name", "")).lower() == "cvx":
        data_dict["vaccine_code_system"] = CVX_CODE_SYSTEM
    data_dict["vaccine_code_text"] = row_dict.get("name", "")
    data_dict["status"] = "completed"
    if row_dict.get("refused", "") == "1":
        data_dict["status"] = "not-done"
    data_dict["occurrence_date_time"] = row_dict.get("date_performed")
    # immunization administered quantity
    if to_float(row_dict.get("administered_amount", "")):
        data_dict["dose_quantity_value"] = to_float(row_dict.get("administered_amount", ""))
        data_dict["dose_quantity_unit"] = row_dict.get("administered_unit", "")
    data_dict["lot_number"] = row_dict.get("vaccine_lot_number", "")
    data_dict["program_eligibility_code"] = row_dict.get("vfc_eligibility_code", "")

    return _transform_resource(
        data_dict=data_dict,
        resource_type=ResourceType.Immunization.value,
        template=IMU_JINJA_TEMPLATE,
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="gap_immunization_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        data_df = data_df.filter(
            (f.col("date_performed") != "")
            & (f.col("date_performed") != "99991231")
            & (f.col("remove_specified_immunization_flag") != "1")
            & (f.col("name") != "")
        )
