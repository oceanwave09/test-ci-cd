    ...
    df = df.select(
        ...
        f.map_from_arrays(
            ...
            f.array(
                ...
                f.col("patient_internal_id"),
                f.col("encounter_internal_id"),
                f.col("condition_onset_date_time"),
                f.col("encounter_date"),
                f.col("code"),
                f.lit(ICT_X_CM_CODE_SYSTEM),
                f.col("name"),
                f.col("status"),
                f.col("pt_condition"),
                f.col("risk"),
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_problemlist_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ecw_problemlist"),
        )
        ...
        data_df = (
            ...
            .withColumn("encounter_date", transform_date_time(f.col("added_date")))
            .withColumn("condition_onset_date_time", transform_date_time(f.col("onset_date")))
            .withColumn("condition_abatement_date_time", transform_date_time(f.col("resolved_on")))
