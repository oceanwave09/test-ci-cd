...
from utils.constants import CPT_CODE_SYSTEM, LONIC_CODE_SYSTEM, PATIENT_MRN_SYSTEM
from utils.enums import ResourceType
from utils.transformation import to_float, transform_date_time
...
def _transform_observation(row_dict: dict, transformer: FHIRTransformer) -> str:
    data_dict = {}
    data_dict["internal_id"] = row_dict.get("observation_internal_id", "")
    data_dict["source_id"] = row_dict.get("order_id", "")
    data_dict["source_file_name"] = row_dict.get("file_format", "")
    data_dict["assigner_organization_id"] = row_dict.get("organization_id", "")
    ...
    if order_type:
        if order_type.upper() == "LAB":
            data_dict["category_code"] = "laboratory"
            data_dict["category_display"] = "Laboratory"
            data_dict["category_system"] = OBS_CATEGORY_SYSTEM
            data_dict["category_text"] = "Laboratory"
        elif order_type.upper() == "XRAY":
            data_dict["category_code"] = "imaging"
            data_dict["category_display"] = "Imaging"
            data_dict["category_system"] = OBS_CATEGORY_SYSTEM
            data_dict["category_text"] = "Imaging"
    data_dict["code_text"] = row_dict.get("order_description", "")
    data_dict["effective_date_time"] = row_dict.get("observation_result_date", "")
    final_components = []
    components = row_dict.get("observation_components", "")
    for component in components:
        entry = {}
        if component.get("lonic"):
            entry["code"] = component.get("lonic")
            entry["code_system"] = LONIC_CODE_SYSTEM
        elif component.get("cpt"):
            entry["code"] = component.get("cpt")
            entry["code_system"] = CPT_CODE_SYSTEM
        entry["code_text"] = component.get("result_description", "")
        if to_float(component.get("result_numeric", "")):
            entry["quantity_value"] = to_float(component.get("result_numeric"))
            entry["quantity_unit"] = component.get("hl_units", "")
        if component.get("result_text"):
            entry["interpretation_text"] = component.get("result_text")
        if component.get("ref_range"):
            ref_range = str(component.get("ref_range"))
            if len(ref_range.split("-")) > 0:
                entry["reference_range_low_value"] = ref_range.split("-")[0].strip()
            if len(ref_range.split("-")) > 1:
                entry["reference_range_high_value"] = ref_range.split("-")[1].strip()
...
def render_resources(row: Row, transformer: FHIRTransformer) -> Row:
    ...
    resources.append(_transform_observation(row_dict, transformer))
    return Row(
        **{
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_orderandresult_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ecw_orderandresult"),
        )
        ...
        data_df = (
            ...
            .withColumn("encounter_date", transform_date_time(f.col("order_date")))
            .withColumn("observation_result_date", transform_date_time(f.col("result_date")))
        )
