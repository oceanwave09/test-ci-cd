...
from utils.constants import ICD_CODE_SYSTEM, PROCEDURE_CPT_SYSTEM, PATIENT_MRN_SYSTEM
...
def _transform_procedure(row_dict: dict, transformer: FHIRTransformer) -> Row:
    ...
    data_dict["location_id"] = row_dict.get("location_internal_id", "")
    data_dict["reason_references"] = [
        {"condition_id": row_dict.get("condition_internal_id_1", "")},
        {"condition_id": row_dict.get("condition_internal_id_2", "")},
        {"condition_id": row_dict.get("condition_internal_id_3", "")},
        {"condition_id": row_dict.get("condition_internal_id_4", "")},
    ]
    data_dict["performed_date_time"] = row_dict.get("encounter_date")
    data_dict["code"] = row_dict.get("cpt_code", "")
    data_dict["code_display"] = row_dict.get("cpt_code_type", "")
    data_dict["code_system"] = PROCEDURE_CPT_SYSTEM
    data_dict["code_text"] = row_dict.get("cpt_code_type", "")
    data_dict["modifiers"] = [
        {"code": row_dict.get("modifier_1", "")},
        {"code": row_dict.get("modifier_2", "")},
        {"code": row_dict.get("modifier_3", "")},
        {"code": row_dict.get("modifier_4", "")},
    ]
...
def fhir_mapper_df(df: DataFrame) -> DataFrame:
    df = df.select(
        ...
        "condition_internal_id_1",
        "condition_internal_id_2",
        "condition_internal_id_3",
        "condition_internal_id_4",
        "encounter_date",
        "cpt_code",
        "cpt_code_type",
        "modifier_1",
        ...
        f.map_from_arrays(
            ...
            f.array(
                f.col("condition_internal_id_1"),
                f.col("patient_internal_id"),
                f.col("encounter_internal_id"),
                f.col("icd_code_1"),
                f.lit(ICD_CODE_SYSTEM),
            ),
        ).alias("condition_rsc_1"),
        f.map_from_arrays(
            f.array(
                f.lit("internal_id"),
                f.lit("patient_id"),
            ...
            f.array(
                f.col("condition_internal_id_2"),
                f.col("patient_internal_id"),
                f.col("encounter_internal_id"),
                f.col("icd_code_2"),
                f.lit(ICD_CODE_SYSTEM),
            ),
        ).alias("condition_rsc_2"),
        f.map_from_arrays(
            f.array(
                f.lit("internal_id"),
                f.lit("patient_id"),
            ...
            f.array(
                f.col("condition_internal_id_3"),
                f.col("patient_internal_id"),
                f.col("encounter_internal_id"),
                f.col("icd_code_3"),
                f.lit(ICD_CODE_SYSTEM),
            ),
        ).alias("condition_rsc_3"),
        f.map_from_arrays(
            f.array(
                f.lit("internal_id"),
                f.lit("patient_id"),
            ...
            f.array(
                f.col("condition_internal_id_4"),
                f.col("patient_internal_id"),
                f.col("encounter_internal_id"),
                f.col("icd_code_4"),
                f.lit(ICD_CODE_SYSTEM),
            ),
        ).alias("condition_rsc_4"),
    )
    return df
...
def render_resources(row: Row, transformer: FHIRTransformer) -> Row:
    ...
    if row_dict.get("condition_rsc_1", {}).get("code"):
        resources.append(
            _transform_resource(
                data_dict=row_dict.get("condition_rsc_1"),
                resource_type=ResourceType.Condition.value,
                template=COND_JINJA_TEMPLATE,
                transformer=transformer,
            )
        )
    if row_dict.get("condition_rsc_2", {}).get("code"):
        resources.append(
            _transform_resource(
                data_dict=row_dict.get("condition_rsc_2"),
                resource_type=ResourceType.Condition.value,
                template=COND_JINJA_TEMPLATE,
                transformer=transformer,
            )
        )
    if row_dict.get("condition_rsc_3", {}).get("code"):
        resources.append(
            _transform_resource(
                data_dict=row_dict.get("condition_rsc_3"),
                resource_type=ResourceType.Condition.value,
                template=COND_JINJA_TEMPLATE,
                transformer=transformer,
            )
        )
    if row_dict.get("condition_rsc_4", {}).get("code"):
        resources.append(
            _transform_resource(
                data_dict=row_dict.get("condition_rsc_4"),
                resource_type=ResourceType.Condition.value,
                template=COND_JINJA_TEMPLATE,
                transformer=transformer,
            )
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_procedurecode_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ecw_procedurecode"),
        )
        ...
        data_df = (
            ...
            .withColumn(
                "location_internal_id",
                f.when(f.col("facility_id") != "", f.expr("uuid()")).otherwise(f.lit("")),
            )
            .withColumn(
                "condition_internal_id_1",
                f.when(f.col("icd_code_1") != "", f.expr("uuid()")).otherwise(f.lit("")),
            )
            .withColumn(
                "condition_internal_id_2",
                f.when(f.col("icd_code_2") != "", f.expr("uuid()")).otherwise(f.lit("")),
            )
            .withColumn(
                "condition_internal_id_3",
                f.when(f.col("icd_code_3") != "", f.expr("uuid()")).otherwise(f.lit("")),
            )
            .withColumn(
                "condition_internal_id_4",
                f.when(f.col("icd_code_4") != "", f.expr("uuid()")).otherwise(f.lit("")),
            )
            .withColumn("procedure_internal_id", f.expr("uuid()"))
            .withColumn("encounter_date", transform_date_time(f.col("service_date")))
