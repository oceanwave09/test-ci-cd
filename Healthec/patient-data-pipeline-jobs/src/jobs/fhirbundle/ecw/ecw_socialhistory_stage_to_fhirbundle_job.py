...
def to_fhir(spark: SparkSession, fhirbundle_landing_path: str, metadata: dict = {}):
    def inner(df: DataFrame):
        ...
        if not src_prov_df.isEmpty():
            prov_df = src_prov_df.transform(render_provider(spark, transformer))
            resources_df = resources_df.union(prov_df)
            df = df.join(src_prov_df, on=prov_cols, how="left").fillna("")
        ...
        if not src_enc_df.isEmpty():
            enc_df = src_enc_df.transform(render_encounter(spark, transformer, metadata))
            resources_df = resources_df.union(enc_df)
            df = df.join(src_enc_df, on=enc_cols, how="left").fillna("")
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_socialhistory_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ecw_socialhistory"),
        )
