...
from utils.constants import ICD10_CODE_SYSTEM, PATIENT_MRN_SYSTEM
...
FAM_HIS_JINJA_TEMPLATE = "family_member_history.j2"
    ...
    df = df.select(
        ...
        f.map_from_arrays(
            f.array(
                ...
                f.lit("name"),
                f.lit("relationship_text"),
                f.lit("deceased_boolean"),
                f.lit("born_date_time"),
                f.lit("age_value"),
                f.lit("code"),
                f.lit("code_system"),
                f.lit("code_display"),
                f.lit("code_text"),
                f.lit("recorded_date_time"),
                f.lit("onset_age_value"),
            ),
            f.array(
                f.col("family_member_history_internal_id"),
                f.col("patient_internal_id"),
                f.col("practitioner_internal_id"),
                f.col("relationship"),
                f.col("relationship"),
                f.col("family_member_history_deceased_boolean"),
                f.col("family_member_history_born_date_time"),
                f.col("family_member_history_age"),
                f.col("icd_code"),
                f.lit(ICD10_CODE_SYSTEM),
                f.col("icd_desc"),
                f.col("icd_desc"),
                f.col("diagnosed_year"),
                f.col("diagnosed_age"),
            ),
        ).alias("family_member_rsc"),
    )
    return df
...
def render_resources(row: Row, transformer: FHIRTransformer) -> Row:
    ...
    resources.append(
        _transform_resource(
            data_dict=row_dict.get("family_member_rsc"),
            resource_type=ResourceType.FamilyMemberHistory.value,
            template=FAM_HIS_JINJA_TEMPLATE,
            transformer=transformer,
        )
    )
    return Row(
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_familyhistory_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ecw_familyhistory"),
        )
        ...
        # transformations
        family_member_history_deceased_boolean_col = f.when(
            f.lower(f.col("status")) == "deceased",
            f.lit("true"),
        ...
        data_df = (
            ...
            .withColumn("family_member_history_internal_id", f.expr("uuid()"))
            .withColumn("family_member_history_deceased_boolean", family_member_history_deceased_boolean_col)
            .withColumn("family_member_history_born_date_time", transform_date(f.col("date_of_birth")))
            .withColumn("family_member_history_age", f.col("age").cast(FloatType()))
            .withColumn("family_member_history_diagnosed_year", transform_date_time(f.col("diagnosed_year")))
            .withColumn("family_member_history_diagnosed_age", f.col("diagnosed_age").cast(FloatType()))
