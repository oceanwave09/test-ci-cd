    ...
    df = df.select(
        ...
        f.map_from_arrays(
            ...
            f.array(
                ...
                f.lit("AMB"),
                f.lit("ambulatory"),
                f.col("modified_surgical_date"),
                f.col("modified_surgical_date"),
            ),
        ).alias("encounter_rsc"),
        f.map_from_arrays(
            f.array(f.lit("reasons")),
            f.array(f.array(f.map_from_entries(f.array(f.struct(f.lit("code_text"), f.col("reason")))))),
        ).alias("reasons"),
        f.map_from_arrays(
            f.array(
                f.lit("internal_id"),
...
def render_resources(row: Row, transformer: FHIRTransformer) -> Row:
    ...
    # add reason codes  to procedure resource
    reasons = row_dict.get("reasons", {})
    get_procedure_resource = row_dict.get("procedure_rsc")
    get_procedure_resource.update(reasons)
    ...
    resources.append(
        _transform_resource(
            data_dict=get_procedure_resource,
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_surgicalhistory_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ecw_surgicalhistory"),
        )
        ...
        data_df = data_df.filter(f.col("cpt_code") != "")
        ...
        data_df = (
            ...
            .withColumn("modified_surgical_date", parse_date_udf(f.col("surgical_date")))
