...
from utils.constants import ICD_CODE_SYSTEM, PATIENT_MRN_SYSTEM
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_diagnosis_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ecw_diagnosis"),
        )
        ...
        # filter rows having "icd_code" value
        data_df = data_df.filter(f.col("icd_code") != "")
        ...
        data_df = (
            ...
            .withColumn("assessment_onset_date", transform_date_time(f.col("assessment_onset_date")))
