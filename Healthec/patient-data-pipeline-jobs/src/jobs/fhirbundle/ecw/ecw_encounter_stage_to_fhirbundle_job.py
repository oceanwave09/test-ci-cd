    ...
    df = df.select(
        ...
        f.map_from_arrays(
            ...
            f.array(
                ...
                f.lit("AMB"),
                f.lit("ambulatory"),
                f.col("encounter_reason"),
                f.col("encounter_visit_type"),
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_encounter_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ecw_encounter"),
        )
        ...
        # transformations
        enc_status_col = f.when(
            f.upper(f.col("encounter_status")) == "CHK",
            f.lit("finished"),
        ).otherwise(f.lit("unknown"))
        enc_start_date_time_col = f.when(
            (f.col("encounter_date") != "") & (f.col("encounter_start_time") != ""),
            f.concat_ws(" ", f.col("encounter_date"), f.col("encounter_start_time")),
        ).otherwise(f.col("encounter_date"))
        enc_end_date_time_col = f.when(
            (f.col("encounter_date") != "") & (f.col("encounter_end_time") != ""),
            f.concat_ws(" ", f.col("encounter_date"), f.col("encounter_end_time")),
        ).otherwise(f.col("encounter_date"))
        ...
        data_df = (
            ...
            .withColumn("encounter_start_date", transform_date_time(enc_start_date_time_col))
            .withColumn("encounter_end_date", transform_date_time(enc_end_date_time_col))
            .withColumn("encounter_status", enc_status_col)
