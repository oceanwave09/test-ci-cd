...
from utils.constants import (
    ...
    PATIENT_MRN_SYSTEM,
    VITAL_CATEGORY_CODE,
    VITAL_CATEGORY_DISPLAY,
    VITAL_CATEGORY_SYSTEM,
    VITAL_CATEGORY_TEXT,
)
...
from utils.utils import (
    ...
    resolve_bp_value,
    resolve_cm_value,
    resolve_height_value,
    resolve_lbs_value,
    resolve_pint_value,
    resolve_vital_values,
    upload_bundle_files,
)
...
VITAL_CODE_DICT = {
    ...
    "pl": ("8867-4", "Heart rate"),
    "spo2": ("59408-5", "Oxygen saturation in Arterial blood by Pulse oximetry"),
    "wt-kg": ("29463-7", "Body weight"),
    "bp-diast": ("8462-4", "Diastolic blood pressure"),
    "bp-syst": ("8480-6", "Systolic blood pressure"),
    "wt": ("29463-7", "Body weight"),
    "ht": ("8302-2", "Body height"),
    "ht-cm": ("8302-2", "Body height"),
    "pt": ("3157-5", "Volume of Blood"),  # TODO: Review with Santhosh
}

VITAL_UNIT_SYSTEM = "http://unitsofmeasure.org"
...
def _transform_vital(row_dict: dict, transformer: FHIRTransformer) -> str:
    ...
    data_dict["category_code"] = VITAL_CATEGORY_CODE
    data_dict["category_display"] = VITAL_CATEGORY_DISPLAY
    data_dict["category_system"] = VITAL_CATEGORY_SYSTEM
    data_dict["category_text"] = VITAL_CATEGORY_TEXT
    data_dict["effective_date_time"] = row_dict.get("encounter_date", "")
    final_components = []
    components = row_dict.get("components", "")
    for component in components:
        standard_vital = component.get("standard_vital", "")
        # resolve vital quantity value
        vitals_value = component.get("vitals_value", "")
        if standard_vital.lower() in ["bmi", "rr", "hr", "pl", "temp", "spo2", "wt-kg"]:
            qty_value = resolve_vital_values(vitals_value)
            if standard_vital.lower() == "wt-kg":
                qty_value = round(qty_value)
        elif standard_vital.lower() == "bp":
            qty_value = resolve_bp_value(vitals_value)
        elif standard_vital.lower() == "wt":
            qty_value = resolve_lbs_value(vitals_value)
        elif standard_vital.lower() == "ht":
            qty_value = resolve_height_value(vitals_value)
        elif standard_vital.lower() == "pt":
            qty_value = resolve_pint_value(vitals_value)
        elif standard_vital.lower() == "ht-cm":
            qty_value = resolve_cm_value(vitals_value)
        else:
            qty_value = ""
        # resolve vital quantity unit
        qty_unit = VITAL_UNIT_DICT.get(standard_vital.lower())[0] if VITAL_UNIT_DICT.get(standard_vital.lower()) else ""
        qty_unit_code = (
            VITAL_UNIT_DICT.get(standard_vital.lower())[1] if VITAL_UNIT_DICT.get(standard_vital.lower()) else ""
        )
        # add vital component
        if qty_value:
            if isinstance(qty_value, list):
                final_components.append(
                    {
                        "code": VITAL_CODE_DICT.get("bp-diast")[0],
                        "code_display": VITAL_CODE_DICT.get("bp-diast")[1],
                        "code_system": LONIC_CODE_SYSTEM,
                        "quantity_value": min(qty_value),
                        "quantity_unit": qty_unit,
                        "quantity_system": VITAL_UNIT_SYSTEM,
                        "quantity_code": qty_unit_code,
                    }
                )
                final_components.append(
                    {
                        "code": VITAL_CODE_DICT.get("bp-syst")[0],
                        "code_display": VITAL_CODE_DICT.get("bp-syst")[1],
                        "code_system": LONIC_CODE_SYSTEM,
                        "quantity_value": max(qty_value),
                        "quantity_unit": qty_unit,
                        "quantity_system": VITAL_UNIT_SYSTEM,
                        "quantity_code": qty_unit_code,
                    }
                ...
                final_components.append(
                    {
                        "code": VITAL_CODE_DICT.get(standard_vital.lower())[0],
                        "code_display": VITAL_CODE_DICT.get(standard_vital.lower())[1],
                        "code_system": LONIC_CODE_SYSTEM,
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_vital_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ecw_vital"),
        )
        ...
        # group by patient id and encounter id
        ...
        ).agg(f.collect_list(f.struct("vitals_name", "standard_vital", "vitals_value")).alias("components"))

        # adding internal id's and meta-data info into existing dataframe
        data_df = (
            ...
            .withColumn(
                "encounter_internal_id", f.when(f.col("encounter_id") != "", f.expr("uuid()")).otherwise(f.lit(""))
            )
            ...
            .withColumn("encounter_date", transform_date_time(f.col("modified_date")))
        )
