...
IMMU_JINJA_TEMPLATE = "immunization.j2"
...
def _transform_immunization(row_dict: dict, transformer: FHIRTransformer) -> Row:
    data_dict = {}
    data_dict["internal_id"] = row_dict.get("immunization_internal_id", "")
    ...
    vaccine_cvx_code = row_dict.get("cvx_code", "")
    vaccine_cpt_code = row_dict.get("cpt_code", "")
    if vaccine_cvx_code:
        data_dict["vaccine_code"] = vaccine_cvx_code
        data_dict["vaccine_system"] = CVX_CODE_SYSTEM
    elif vaccine_cpt_code:
        data_dict["vaccine_code"] = vaccine_cpt_code
        data_dict["vaccine_system"] = CPT_CODE_SYSTEM
    data_dict["vaccine_text"] = row_dict.get("immunization_description", "")
    data_dict["extensions"] = []
    if row_dict.get("vaccine_manufacturer") != "":
        data_dict["extensions"].append(
            {"url": VACCINE_MANUFACTURER_URL, "value": row_dict.get("vaccine_manufacturer", "")}
        )
    data_dict["practitioner_function_code"] = "RENDERING"
    if row_dict.get("immunization_dose", ""):
        parsed_quantity = row_dict.get("immunization_dose", "").split()
        if len(parsed_quantity) > 0 and to_float(parsed_quantity[0]):
            data_dict["dose_quantity_value"] = to_float(parsed_quantity[0])
            if len(parsed_quantity) > 1:
                data_dict["dose_quantity_unit"] = parsed_quantity[1]
    data_dict["occurrence_date_time"] = row_dict.get("encounter_date", "")
    data_dict["lot_number"] = row_dict.get("lot_number", "")
    data_dict["route_text"] = row_dict.get("route", "")
    data_dict["site_text"] = row_dict.get("site", "")
    data_dict["expiration_date"] = row_dict.get("immunization_expiration_date", "")
    data_dict["status"] = (
        "completed" if row_dict.get("immunization_status", "").lower() == "administered" else "not-done"
    )
    data_dict["program_eligibility_text"] = row_dict.get("vfc_status", "")

    return _transform_resource(
        data_dict=data_dict,
        template=IMMU_JINJA_TEMPLATE,
        resource_type=ResourceType.Immunization.value,
...
def render_resources(row: Row, transformer: FHIRTransformer) -> Row:
    ...
    resources.append(_transform_immunization(row_dict, transformer))
    return Row(
        **{
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_immunization_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ecw_immunization"),
        )
        ...
        data_df = (
            ...
            .withColumn("immunization_internal_id", f.expr("uuid()"))
            ...
            .withColumn("encounter_date", transform_date_time(f.col("given_date")))
            .withColumn("immunization_expiration_date", transform_date(f.col("expiration_date")))
        )
