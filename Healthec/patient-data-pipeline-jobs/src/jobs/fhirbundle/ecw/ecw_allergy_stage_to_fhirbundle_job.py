    ...
    df = df.select(
        ...
        f.map_from_arrays(
            ...
            f.array(
                ...
                f.col("allergen_type"),
                f.col("allergy_reaction_text"),
                f.col("modified_date"),
                f.col("allergy_id"),
                f.lit(NDC_CODE_SYSTEM),
                f.when(f.col("allergy_id") != "", f.lit("medication")).otherwise(f.lit("")),
                f.col("allergy_description"),
                f.col("allergy_status"),
                f.col("allergies_verified"),
            ),
        ).alias("allergy_rsc"),
    )
    return df
...
@click.option(
    ...
    "-a",
    help="spark app name",
    default="ecw_allergy_stage_to_fhirbundle_job",
)
@click.option(
    "--config-file-path",
    "-c",
...
def main(app_name, config_file_path=None):
    ...
    try:
        ...
        delta_table_name = os.environ.get(
            "DELTA_TABLE_NAME",
            config.get("delta_table_name", "ecw_allergy"),
        )
        ...
        # transformations
        alrgy_type_col = f.when(
            f.lower(f.col("allergen_type")).isin(["allergy", "intolerance"]), f.lower(f.col("allergen_type"))
        ).otherwise(f.lit("allergy"))
        alrgy_status_col = f.when(f.lower(f.col("allergy_status")) == "inactive", "inactive").otherwise(f.lit("active"))
        alrgy_verfy_col = f.when(
            f.upper(f.col("allergies_verified")) == "Y",
            f.lit("confirmed"),
        ...
        data_df = (
            ...
            .withColumn("allergen_type", alrgy_type_col)
            .withColumn("allergy_status", alrgy_status_col)
            .withColumn("allergies_verified", alrgy_verfy_col)
            .withColumn("encounter_date", transform_date_time(f.col("encounter_date")))
            .withColumn("modified_date", transform_date_time(f.col("modified_date")))
