CREATE TABLE IF NOT EXISTS nucleo_analytical.procedure(
    procedure_fhir_id VARCHAR,
    external_id VARCHAR,
    status VARCHAR,
    category_code VARCHAR,
    category_system VARCHAR,
    category_desc VARCHAR,
    code VARCHAR,
    code_desc VARCHAR,
    code_system VARCHAR,
    patient_fhir_id VARCHAR,
    encounter_fhir_id VARCHAR,
    performed_start_date TIMESTAMP(3) WITH TIME ZONE,
    performed_end_date TIMESTAMP(3) WITH TIME ZONE,
    provider_fhir_id VARCHAR,
    facility_fhir_id VARCHAR,
    reason_fhir_id VARCHAR,
    modifier_url VARCHAR,
    modifier_code VARCHAR,
    modifier_desc VARCHAR,
    modifier_system VARCHAR,
    reason_code VARCHAR,
    reason_system VARCHAR,
    reason_desc VARCHAR,
    bodysite_code VARCHAR,
    bodysite_system VARCHAR,
    bodysite_desc VARCHAR,
    outcome_code VARCHAR,
    outcome_system VARCHAR,
    outcome_desc VARCHAR,
    updated_user VARCHAR,
    updated_ts TIMESTAMP(3) WITH TIME ZONE
)
WITH (
    LOCATION = 's3a://phm-development-datapipeline-bucket/delta-tables/nucleo/analytical/procedure',
    CHECKPOINT_INTERVAL = 5
);