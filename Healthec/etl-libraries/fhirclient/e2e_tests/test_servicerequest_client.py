...
from fhirclient.resources.servicerequest import ServiceRequest
from e2e_tests import base, utils

logging.basicConfig(level=logging.INFO)


class TestServiceRequestClient:

    @pytest.fixture
    def new_resource(self) -> str:
        with open("e2e_tests/data/servicerequest.json", "r") as f:
    ...
    def update_field(self) -> dict:
        with open("e2e_tests/data/servicerequest_status.json", "r") as f:
    ...
    def setup_method(cls):
        ...
        cls.enc_id = enc_id
        # create servicerequest client
        cls.client = ServiceRequest(fhir_client)

    # TODO: skips test since the service request create api endpoint has not exposed
    # @pytest.mark.skip()
    def test_create_servicerequest(self, new_resource):
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
        assert _id is not None

    # TODO: skips test since the service request get api endpoint has not exposed
    # @pytest.mark.skip()
    def test_get_servicerequest(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    # TODO: skips test since the service request update api endpoint has not exposed
    # @pytest.mark.skip()
    def test_update_servicerequest(self, new_resource, update_field):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    # TODO: skips test since the service request match api endpoint has not exposed
    @pytest.mark.skip()
    def test_match_servicerequest(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
