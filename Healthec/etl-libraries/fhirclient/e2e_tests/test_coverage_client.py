...
class TestCoverageClient:
    ...
    def update_field(self) -> dict:
        with open("e2e_tests/data/coverage_period.json", "r") as f:
            return json.load(f)
    ...
    def setup(cls):
        ...
        cls.client = Coverage(fhir_client, scope_id=pat_id)

    def test_create_coverage(self, new_resource):
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
        assert _id is not None

    def test_get_coverage(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    def test_update_coverage(self, new_resource, update_field):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    # TODO: skips test since the coverage match api endpoint has not exposed
    @pytest.mark.skip()
    def test_match_coverage(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
