...
class TestPatientClient:
    ...
    def update_field(self) -> dict:
        with open("e2e_tests/data/patient_contact.json", "r") as f:
            return json.load(f)
    ...
    def setup_method(cls):
        ...
        cls.client = Patient(fhir_client)

    def test_create_patient(self, new_resource):
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
        assert _id is not None

    def test_create_or_update_patient(self, new_resource):
        resp_data = self.client.upsert(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
        assert _id is not None

    def test_get_patient(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    def test_update_patient(self, new_resource, update_field):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    def test_match_patient(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
