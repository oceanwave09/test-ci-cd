...
class TestMedicationRequestClient:
    ...
    def update_field(self) -> dict:
        with open("e2e_tests/data/medicationrequest_status.json", "r") as f:
    ...
    def setup_method(cls):
        ...
        cls.enc_id = enc_id
        # create medicationrequest client
        cls.client = MedicationRequest(fhir_client)

    # TODO: skips test since the medication request create api endpoint has not exposed
    # @pytest.mark.skip()
    def test_create_medicationrequest(self, new_resource):
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
        assert _id is not None

    # TODO: skips test since the medication request get api endpoint has not exposed
    # @pytest.mark.skip()
    def test_get_medicationrequest(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    # TODO: skips test since the medication request update api endpoint has not exposed
    # @pytest.mark.skip()
    def test_update_medicationrequest(self, new_resource, update_field):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    # TODO: skips test since the medication request match api endpoint has not exposed
    @pytest.mark.skip()
    def test_match_medicationrequest(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
