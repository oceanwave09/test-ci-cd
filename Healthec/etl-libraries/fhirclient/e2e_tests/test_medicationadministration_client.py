...
from fhirclient.resources.medicationadministration import MedicationAdministration
from e2e_tests import base, utils

logging.basicConfig(level=logging.INFO)


class TestMedicationAdministrationClient:

    @pytest.fixture
    def new_resource(self) -> str:
        with open("e2e_tests/data/medicationadministration.json", "r") as f:
                template = Template(f.read())
        values = {}
        values["pat_id"] = self.pat_id
        values["medi_id"] = self.medi_id
        return template.safe_substitute(values)

    @pytest.fixture
    def update_field(self) -> dict:
        with open("e2e_tests/data/medicationadministration_status.json", "r") as f:
            return json.load(f)
    ...
    def setup_method(cls):
        ...
        medi_client = Medication(fhir_client)
        with open("e2e_tests/data/medication.json", "r") as f:
                content = f.read()
        resp_data = medi_client.create(content)
        medi_id = resp_data.get("id")
        cls.medi_id = medi_id
        # create medication administration client
        cls.client = MedicationAdministration(fhir_client, scope_id=pat_id)

    def test_create_medicationadministration(self, new_resource):
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
        assert _id is not None

    def test_get_medicationadministration(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    def test_update_medicationadministration(self, new_resource, update_field):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    # TODO: skips test since the medication administration match api endpoint has not exposed
    @pytest.mark.skip()
    def test_match_medicationadministration(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
