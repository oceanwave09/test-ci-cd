...
class TestClaimClient:
    ...
    def new_resource(self) -> str:
        with open("e2e_tests/data/claim.json", "r") as f:
                template = Template(f.read())
        values = {}
        values["patient_id"] = self.pat_id
        values["organization_id"] = self.org_id
        values["coverage_id"] = self.coverage_id
        return template.safe_substitute(values)

    @pytest.fixture
    def update_field(self) -> dict:
        with open("e2e_tests/data/claim_identifier.json", "r") as f:
            return json.load(f)
    ...
    def setup(cls):
        ...
        coverage_client = Coverage(fhir_client)
        with open("e2e_tests/data/coverage.json", "r") as f:
                template = Template(f.read())
        values = {}
        values['patient_id'] = pat_id
        values['organization_id'] = org_id
        new_coverage_resource = template.safe_substitute(values)
        resp_data = coverage_client.create(new_coverage_resource)
        coverage_id = resp_data.get("id")
        cls.coverage_id = coverage_id
        # create claim  client
        cls.client = Claim(fhir_client, scope_id=pat_id)

    def test_create_claim(self, new_resource):
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
        assert _id is not None

    def test_get_claim(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    def test_update_claim(self, new_resource, update_field):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    # TODO: skips test since the claim match api endpoint has not exposed
    @pytest.mark.skip()
    def test_match_claim(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
