...
class TestAllergyIntoleranceClient:

    @pytest.fixture
    def new_resource(self) -> str:
        with open("e2e_tests/data/allergyintolerance.json", "r") as f:
    ...
    def update_field(self) -> dict:
        with open("e2e_tests/data/allergyintolerance_verify.json", "r") as f:
            return json.load(f)
    ...
    def setup_method(cls):
        ...
        cls.pat_id = pat_id
        # create allergy intolerance client
        cls.client = AllergyIntolerance(fhir_client, scope_id=pat_id)

    def test_create_allergyintolerance(self, new_resource):
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
        assert _id is not None

    def test_get_allergyintolerance(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    def test_update_allergyintolerance(self, new_resource, update_field):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    # TODO: skips test since the allergy intolerance match api endpoint has not exposed
    @pytest.mark.skip()
    def test_match_allergyintolerance(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
