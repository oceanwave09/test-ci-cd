...
class TestMedicationStatementClient:

    @pytest.fixture
    def new_resource(self) -> str:
        with open("e2e_tests/data/medicationstatement.json", "r") as f:
    ...
    def update_field(self) -> dict:
        with open("e2e_tests/data/medicationstatement_status.json", "r") as f:
            return json.load(f)
    ...
    def setup_method(cls):
        ...
        cls.pat_id = pat_id
        # create medication statement client
        cls.client = MedicationStatement(fhir_client, scope_id=pat_id)

    def test_create_medicationstatement(self, new_resource):
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
        assert _id is not None

    def test_get_medicationstatement(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    def test_update_medicationstatement(self, new_resource, update_field):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    # TODO: skips test since the medication statement match api endpoint has not exposed
    @pytest.mark.skip()
    def test_match_medicationstatement(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
