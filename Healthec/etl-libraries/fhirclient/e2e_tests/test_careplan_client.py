...
from fhirclient.resources.careplan import CarePlan
from e2e_tests import base, utils

logging.basicConfig(level=logging.INFO)


class TestCarePlanClient:

    @pytest.fixture
    def new_resource(self) -> str:
        with open("e2e_tests/data/careplan.json", "r") as f:
    ...
    def update_field(self) -> dict:
        with open("e2e_tests/data/careplan_status.json", "r") as f:
            return json.load(f)
    ...
    def setup_method(cls):
        ...
        # create care plan client
        cls.client = CarePlan(fhir_client)

    # TODO: skips test since the care plan create api endpoint has not exposed
    @pytest.mark.skip()
    def test_create_careplan(self, new_resource):
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
        assert _id is not None

    # TODO: skips test since the care lan get api endpoint has not exposed
    @pytest.mark.skip()
    def test_get_careplan(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    # TODO: skips test since the care plan update api endpoint has not exposed
    @pytest.mark.skip()
    def test_update_careplan(self, new_resource, update_field):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    # TODO: skips test since the care plan match api endpoint has not exposed
    @pytest.mark.skip()
    def test_match_careplan(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
