...
from fhirclient.resources.episodeofcare import EpisodeOfCare
from e2e_tests import base, utils

logging.basicConfig(level=logging.INFO)


class TestEpisodeOfCareClient:

    @pytest.fixture
    def new_resource(self) -> str:
        with open("e2e_tests/data/episodeofcare.json", "r") as f:
    ...
    def update_field(self) -> dict:
        with open("e2e_tests/data/episodeofcare_status.json", "r") as f:
            return json.load(f)
    ...
    def setup_method(cls):
        ...
        cls.pat_id = pat_id
        # create episodeofcare client
        cls.client = EpisodeOfCare(fhir_client, scope_id=pat_id)

    def test_create_episodeofcare(self, new_resource):
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
        assert _id is not None

    # TODO: skips test since the episodeofcare get api endpoint has issue
    @pytest.mark.skip()
    def test_get_episodeofcare(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    # TODO: skips test since the episodeofcare update api endpoint has issue
    @pytest.mark.skip()
    def test_update_episodeofcare(self, new_resource, update_field):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
    ...
    # TODO: skips test since the episodeofcare match api endpoint has not exposed
    @pytest.mark.skip()
    def test_match_episodeofcare(self, new_resource):
        # create resource
        resp_data = self.client.create(new_resource)
        assert resp_data is not None
        _id = resp_data.get("id")
