...
class DiagnosticReport(Base):
    """DiagnosticReport resource API client"""
    ...
    def __init__(
        ...
        resource_path = None
        if scope_id:
            resource_path = ResourceType.DiagnosticReport.get_resource_path(scope, scope_id)
        super().__init__(
            client,
            ResourceType.DiagnosticReport.value,
            scope=scope,
            scope_id=scope_id,
    ...
    def _update_resource_reference(self, resource: dict) -> dict:
        ...
        # update observation internal reference
        if resource.get("result") and len(resource.get("result")) > 0:
            client = Observation(self._client, scope_id=self._scope_id)
            updated_references = []
            references = resource.get("result")
            for reference in references:
                updated_references.append(client._update_reference(reference))
            resource["result"] = updated_references
    ...
    def _resolve_scope(self, resource: dict) -> None:
        if resource.get("subject"):
            ...
            self._resource_path = ResourceType.DiagnosticReport.get_resource_path(
                "Patient",
                self._scope_id
            )
