...
class ServiceRequest(Base):
    """ServiceRequest resource API client"""
    ...
    def __init__(
        ...
        resource_path = None
        if scope_id:
            resource_path = ResourceType.ServiceRequest.get_resource_path(scope, scope_id)
        super().__init__(
            client,
            ResourceType.ServiceRequest.value,
            scope=scope,
            scope_id=scope_id,
    ...
    def _resolve_scope(self, resource: dict) -> None:
        if resource.get("subject"):
            ...
            self._resource_path = ResourceType.ServiceRequest.get_resource_path(
                "Patient",
                self._scope_id
            )
