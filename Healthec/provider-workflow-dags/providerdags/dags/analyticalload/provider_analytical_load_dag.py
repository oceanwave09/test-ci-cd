...
def _prepare_run_template(ti):
    ...
    src_template = os.path.join(TMPL_SEARCH_PATH, "provider", "analyticalload", "provider_analytical_load_job.yaml")
...
with DAG(
    dag_id="provider_analytical_load_dag",
    default_args=default_args,
    schedule_interval=None,
    max_active_runs=1,
    concurrency=1,
    description="Loads provider data into analytical table in delta lake",
    template_searchpath=[TMPL_SEARCH_PATH],
    tags=["analyticalload", "provider"],
) as dag:
