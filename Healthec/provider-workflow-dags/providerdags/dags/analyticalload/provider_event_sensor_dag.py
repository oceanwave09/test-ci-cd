...
from providerdags.utils.constants import EVENT_FIELDS
...
def _init_dag(ti):
    ...
    event_file_path = os.path.join(
        ...
        "files/ANALYTICAL/PROVIDER/landing/_TENANT_",
        f'provider_topic_events_{datetime.now(timezone.utc).strftime("%Y_%m_%d_%H_%M_%S")}.csv',
    )
...
def _consume_events(start_ts: str, event_file_path: str):
    ...
    # read kafka provider topic variables
    servers = Variable.get("KAFKA_SERVERS")
    topic = Variable.get("PROVIDER_TOPIC_NAME")
    ...
    conf = {
        ...
        "group.id": "analytical_load_provider_group",
...
with DAG(
    dag_id="provider_event_sensor_dag",
    default_args=default_args,
    schedule_interval="*/30 * * * *",
    max_active_runs=1,
    catchup=False,
    description=(
        "Runs for every 30 minutes, collects provider events from provider kafka topic and "
        "writes events as CSV file into S3 bucket analytical directory"
    ),
    render_template_as_native_obj=True,
    tags=["sensor", "kafka", "provider"],
) as dag:
