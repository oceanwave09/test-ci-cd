...
from providerdags.utils.utils import list_files_from_messages, trigger_stage_load_dag
...
with DAG(
    dag_id="provider_stage_load_sensor_dag",
    default_args=default_args,
    schedule_interval="*/5 * * * *",
    max_active_runs=1,
    catchup=False,
    description="Runs for every 5 minutes and triggers provider stage load dags \
        for each new file placed in canonical S3 landing bucket",
    render_template_as_native_obj=True,
    tags=["sensor", "stageload", "provider"],
) as dag:
    sqs_sensor = SqsSensor(
        task_id="sqs_sensor",
        sqs_queue=Variable.get("PROVIDER_STAGE_LOAD_SQS_QUEUE_URL"),
        max_messages=10,
        num_batches=3,
        mode="reschedule",
        soft_fail=True,
