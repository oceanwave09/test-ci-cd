...
def read_file_from_s3(bucket: str, key: str) -> tuple:
    try:
        ...
        publish_error_code(f"{PatientDagsErrorCodes.AWS_CLIENT_ERROR.value}: {str(e)}")
...
def list_file_from_s3(bucket: str, prefix: str) -> list:
    try:
        print(f"list_file_from_s3 - bucket: {bucket}, prefix: {prefix}")
        client = boto3.client("s3")
        response = client.list_objects_v2(Bucket=bucket, Prefix=prefix)
        file_contents = response.get("Contents")
        files = [f"s3://{bucket}/{entry['Key']}" for entry in file_contents if entry["Size"] > 0]
        print(f"list_file_from_s3 - files: {files}")
        return files
    except ClientError as e:
        publish_error_code(f"{PatientDagsErrorCodes.AWS_CLIENT_ERROR.value}: {str(e)}")
        raise AirflowFailException(e)
