...
def _init_dag(ti):
    ...
    event_file_path = os.path.join(
        ...
        "files/FHIRBUNDLE_BULK_STATUS/_TENANT_/landing/",
        f'fhirbundle_status_topic_events_{datetime.now(timezone.utc).strftime("%Y_%m_%d_%H_%M_%S")}.csv',
    )
...
def _consume_events(start_ts: str, event_file_path: str):
    ...
    # read kafka fhirbundle_status topic variables
    servers = Variable.get("KAFKA_SERVERS")
    topic = Variable.get("FHIRBUNDLE_STATUS_TOPIC_NAME")
    ...
    conf = {
        ...
        # "group.id": "analytical_load_patient_group", # to do: check with team
    ...
    try:
        ...
        while event_count < max_batch_messages:
            ...
            if msg.error():
                ...
                tenant = json_data.get("file_tenant")
        ...
        # write events as CSV file into S3 bucket fhirbundle bulk status directory if events exists in the list
...
with DAG(
    dag_id="fhirbundle_status_event_sensor_dag",
    default_args=default_args,
    schedule_interval="*/30 * * * *",
    max_active_runs=1,
    catchup=False,
    description=(
        "Runs for every 30 minutes, collects fhirbundle status events from fhirbundle status kafka topic and "
        "writes events as CSV file into S3 bucket fhirbundle status directory"
    ),
    render_template_as_native_obj=True,
    tags=["sensor", "kafka", "fhirbundle"],
) as dag:
