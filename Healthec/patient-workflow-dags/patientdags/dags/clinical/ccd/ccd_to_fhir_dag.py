...
from patientdags.ccdaparser.converter import CcdaToFhir
...
def _ccda_to_fhir(bucket: str, src_key: str, dest_key: str, canonical_key: str, metadata: dict = {}):
    try:
        src_file_path = f"s3://{bucket}/{src_key}"
        dest_file_path = f"s3://{bucket}/{dest_key}"
        canonical_file_path = f"s3://{bucket}/{canonical_key}"
        converter = CcdaToFhir(src_file_path, canonical_file_path)
        converter.convert(dest_file_path, metadata)
    except Exception as e:
        publish_error_code(f"{PatientDagsErrorCodes.CCD_TO_FHIR_CONVERSION_ERROR.value}: {str(e)}")
        raise AirflowFailException(e)


with DAG(
    dag_id="ccd_to_fhir_dag",
    default_args=default_args,
    schedule_interval=None,
    max_active_runs=2,
    concurrency=2,
    dagrun_timeout=timedelta(minutes=30),
    description="Converts CCD to FHIR bundle and write it into S3 bucket",
    tags=["ccd"],
) as dag:
    ...
    convert_ccd_to_fhirbundle = PythonOperator(
        task_id="convert_ccd_to_fhirbundle",
        python_callable=_ccda_to_fhir,
        op_kwargs={
    ...
    (parse_config >> init_dag >> convert_ccd_to_fhirbundle >> post_dag_status)
