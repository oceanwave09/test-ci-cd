...
from patientdags.utils.constants import MEASURE_EVENT_FIELDS
...
def _init_dag(ti):
    ...
    event_file_path = os.path.join(
        ...
        "files/ANALYTICAL/MEASURE/landing/_TENANT_",
        f'measure_topic_events_{datetime.now(timezone.utc).strftime("%Y_%m_%d_%H_%M_%S")}.csv',
    )
...
def _write_events_into_s3(events: list, file_path: str):
    with open(file_path, "w") as event_file:
        event_writer = csv.DictWriter(event_file, fieldnames=MEASURE_EVENT_FIELDS)
...
def _consume_events(start_ts: str, event_file_path: str):
    ...
    # read kafka patient topic variables
    servers = Variable.get("KAFKA_SERVERS")
    topic = Variable.get("PATIENT_TOPIC_NAME")
    ...
    conf = {
        ...
        "group.id": "analytical_load_measure_group",
    ...
    try:
        ...
        while event_count < max_batch_messages:
            ...
            if msg.error():
                ...
                if json_data.get("event", "") in ["calculate:quality_measure"] and json_data.get("status", "") in [
                    "success"
                ]:
...
with DAG(
    dag_id="measure_event_sensor_dag",
    default_args=default_args,
    schedule_interval="*/30 * * * *",
    max_active_runs=1,
    catchup=False,
    description=(
        "Runs for every 30 minutes, collects measure events from patient kafka topic and "
        "writes events as CSV file into S3 bucket analytical directory"
    ),
    render_template_as_native_obj=True,
    tags=["sensor", "kafka", "measure"],
) as dag:
