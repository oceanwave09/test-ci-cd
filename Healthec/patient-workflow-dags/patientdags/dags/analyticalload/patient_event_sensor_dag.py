...
def _init_dag(ti):
    ...
    event_file_path = os.path.join(
        ...
        "files/ANALYTICAL/PATIENT/landing/_TENANT_",
        f'patient_topic_events_{datetime.now(timezone.utc).strftime("%Y_%m_%d_%H_%M_%S")}.csv',
    )
...
def _consume_events(start_ts: str, event_file_path: str):
    ...
    conf = {
        ...
        "group.id": "analytical_load_patient_group",
    ...
    try:
        ...
        while event_count < max_batch_messages:
            ...
            if msg.error():
                ...
                if (
                    json_data.get("origin", "") in ["patient-service"]
                    and json_data.get("resourceType", "") not in ["PatientBundle"]
                    and json_data.get("event", "") in ["create", "update"]
                ):
...
with DAG(
    dag_id="patient_event_sensor_dag",
    default_args=default_args,
    schedule_interval="*/30 * * * *",
    max_active_runs=1,
    catchup=False,
    description=(
        "Runs for every 30 minutes, collects patient events from patient kafka topic and "
        "writes events as CSV file into S3 bucket analytical directory"
    ),
    render_template_as_native_obj=True,
    tags=["sensor", "kafka", "patient"],
) as dag:
