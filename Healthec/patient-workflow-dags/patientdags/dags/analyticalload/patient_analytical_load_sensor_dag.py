...
with DAG(
    dag_id="patient_analytical_load_sensor_dag",
    default_args=default_args,
    schedule_interval="*/5 * * * *",
    max_active_runs=1,
    catchup=False,
    description=(
        "Runs for every 5 minutes and triggers patient analytical load dags for each new file placed "
        "in analytical S3 landing bucket directory"
    ),
    render_template_as_native_obj=True,
    tags=["sensor", "analyticalload", "patient"],
) as dag:
    sqs_sensor = SqsSensor(
        task_id="sqs_sensor",
        sqs_queue=Variable.get("PATIENT_ANALYTICAL_LOAD_SQS_URL"),
        max_messages=10,
        num_batches=3,
        mode="reschedule",
        soft_fail=True,
