...
def _prepare_run_template(ti):
    ...
    if file_format.lower() != "gap":
        raise ValueError("Invalid file format")

    src_template = os.path.join(TMPL_SEARCH_PATH, "patient", "stageload", "gap_stage_load_job.yaml")
...
with DAG(
    dag_id="gap_stage_load_dag",
    default_args=default_args,
    schedule_interval=None,
    max_active_runs=2,
    concurrency=2,
    dagrun_timeout=timedelta(minutes=90),
    description="Loads canonical gap data into staging table in delta lake",
