...
def _prepare_run_template(ti):
    ...
    if file_format.lower() != "ncqa_hedis":
        raise ValueError("Invalid file format")

    src_template = os.path.join(TMPL_SEARCH_PATH, "patient", "stageload", "ncqa_hedis_stage_load_job.yaml")
...
with DAG(
    dag_id="ncqa_hedis_stage_load_dag",
    default_args=default_args,
    schedule_interval=None,
    max_active_runs=2,
    concurrency=2,
    description="Loads canonical ncqa hedis data into staging table in delta lake",
    template_searchpath=[TMPL_SEARCH_PATH],
    tags=["stageload", "ncqa_hedis"],
) as dag:
