...
from patientdags.utils.constants import RADAID_STAGE_TO_FHIR_DAG, TMPL_SEARCH_PATH
...
def _prepare_run_template(ti):
    ...
    if file_format.lower() != "radaid":
        raise ValueError("Invalid file format")

    src_template = os.path.join(TMPL_SEARCH_PATH, "patient", "stageload", "radaid_stage_load_job.yaml")
...
def _trigger_fhir_bundle(ti):
    ...
    trigger_dag = RADAID_STAGE_TO_FHIR_DAG
    trigger_dag_with_config(dag_to_trigger=trigger_dag, conf=fhir_config)


with DAG(
    dag_id="radaid_stage_load_dag",
    default_args=default_args,
    schedule_interval=None,
    max_active_runs=2,
    concurrency=2,
    description="Loads canonical radaid data into staging table in delta lake",
    template_searchpath=[TMPL_SEARCH_PATH],
    tags=["stageload", "radaid"],
) as dag:
    ...
    (parse_config >> init_dag >> stage_load >> trigger_fhir_bundle >> post_dag_status)
