...
from patientdags.utils.constants import (  # CCLF_BENEFICIARY_TYPES,
...
def _init_cclf_bene_canonical_files(ti, file_name: str):
    file_seq = file_name.split(".")[-2].split("-")[-1]
    file_type = f"ALR1_{file_seq}"
...
def _convert_cclf_bene_to_canonical(ti, local_path: str = DOWNLOAD_ZIP_FILE_PATH) -> tuple:
    ...
    resource_type = f"CCLF_BENEFICIARY_{file_type}"
    metadata = {
...
def _prepare_run_template(ti):
    ...
    if file_format.upper() != "CCLF_BENEFICIARY":
        raise ValueError("Invalid file format")

    src_template = os.path.join(TMPL_SEARCH_PATH, "patient", "stageload", "cclf_beneficiary_stage_load_job.yaml")
    ...
    substitute = {
        ...
        "ENTITYNAME": file_type.replace("_", "-").lower(),
    }
...
@dag(
    dag_id="cclf_beneficiary_stage_load_dag",
    start_date=datetime(day=2, year=2022, month=8),
    schedule=None,
    max_active_runs=1,
    concurrency=3,
    description="Loads canonical cclf beneficiary data into staging table in delta lake",
    render_template_as_native_obj=True,
    template_searchpath=[TMPL_SEARCH_PATH],
    tags=["stageload", "cclf_beneficiary"],
)
def cclf_beneficiary_stage_load():
    ...
    @task
    def extract_file_names(file_key):
        file_names = []
        client = boto3.client("s3")
        bucket_name = Variable.get("DATA_INGESTION_BUCKET")
        response = client.get_object(Bucket=bucket_name, Key=file_key)
    ...
    def stage_load_group(file_name):
        init_canonical_files = PythonOperator(
            task_id="init_group",
            python_callable=_init_cclf_bene_canonical_files,
            op_kwargs={
                "file_name": file_name,
            },
        ...
        convert_raw_to_canonical = PythonOperator(
            ...
            python_callable=_convert_cclf_bene_to_canonical,
            provide_context=True,
        )
    ...
    def trigger_fhir_bundle():
        ...
        trigger_dag = "cclf_beneficiary_stage_to_fhirbundle_dag"
    ...
    file_names = extract_file_names(config_dict["file_key"])

    stage_load_group.expand(file_name=file_names) >> trigger_fhir_bundle() >> post_dag_status()


result = cclf_beneficiary_stage_load()
